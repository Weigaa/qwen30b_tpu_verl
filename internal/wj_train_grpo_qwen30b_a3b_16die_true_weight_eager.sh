...
export VLLM_ASCEND_MODE1_PARITY_KEEP_FLOOR4_GROUP_CACHE=${VLLM_ASCEND_MODE1_PARITY_KEEP_FLOOR4_GROUP_CACHE:-1}
...
export VLLM_ASCEND_MODE5_SINGLE_CONTROL_MESSAGE_REMOTE=${VLLM_ASCEND_MODE5_SINGLE_CONTROL_MESSAGE_REMOTE:-1}
    ...
    # Mode=4/5 remote-cache paths should not reserve broad communication
    # workspace by default. The only default KV-sizing cost should be the
    # runtime double buffer; set these envs explicitly when validating a
    # known low-floor HCCL workspace issue.
    export VLLM_ASCEND_MODE4_MOE_DISPATCH_HEADROOM_BYTES=${VLLM_ASCEND_MODE4_MOE_DISPATCH_HEADROOM_BYTES:-0}
