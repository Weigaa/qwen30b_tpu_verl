...
# This script is specifically for the ziyi-aligned TP+EP diagnostic path.
# VLLM_ROLLOUT_PARALLEL_MODE is only a mode label in our shell stack; the
# rollout TP actually takes effect through ROLLOUT_TP_SIZE below.
...
export VLLM_ROLLOUT_FAST_DEBUG=${VLLM_ROLLOUT_FAST_DEBUG:-0}
