...
MODEL_PATH=${MODEL_PATH:-"/home/data/Qwen3-32B"}
...
python3 -m verl.trainer.main_ppo --config-path="${CONFIG_DIR}" \
    ...
    +trainer.rollout_data_dir=/workspace/data/dump \
    +trainer.rollout_length_dir=/workspace/data/dump \
    ...
    +actor_rollout_ref.rollout.engine_kwargs.vllm.speculative_config.num_speculative_tokens=3 "$@" >> qwen32b-record.txt \
    2> >(grep -Fv "TBE Subprocess[task_distribute] raise error" >&2)
