...
- keep the reusable full-world `_EP` cache
- drop stale MC2 cache entries instead of keeping old floor-world/current-world mixes
...
8. optionally drop stale cached groups after shrink
...
By default, mode=1 does not drop full-world groups immediately after shrink.
The full-world group is useful for the following restore. Stale floor-world
groups are cleaned after full-world restore.
...
8. drop stale cached groups with `keep_group_ranks=tuple(range(world_size))`
...
- do not keep stale MC2:
  `VLLM_ASCEND_MODE1_PARITY_KEEP_MC2_GROUP_CACHE=0`
- after restore, drop stale floor MC2 groups
...
Full-world EP is the one stale group intentionally kept during shrink:
...
- keeping full-world EP avoids paying the full recreate / warmup cost again
- unlike stale floor MC2, this cache has a clear reuse point on restore

Only the full-world EP signature should be kept this way. Stale floor EP / DP
groups should not accumulate. Mode=1 does not keep stale DP groups by default.
...
`VLLM_ASCEND_MODE1_PARITY_DROP_STALE_CACHE_AFTER_SHRINK=0` does **not** mean
"never delete stale MC2". It only disables the immediate cache sweep right
after shrink, because at that moment the old full-world MC2 is stale relative
to the floor group but is still useful for the coming restore.

The stale floor MC2 is still deleted after restore:

```text
restore to keep_ranks=(0, 1, ..., 15)
  -> _drop_stale_cached_elastic_parallel_groups(tuple(range(world_size)))
  -> floor MC2 ranks != full-world ranks
  -> VLLM_ASCEND_MODE1_PARITY_KEEP_MC2_GROUP_CACHE=0
  -> stale floor MC2 is destroyed
```
...
These force the runtime closer to "destroy and recreate communicators every
cycle", which is exactly what made restore communication blow up to hundreds of
seconds.
...
- suspicious env banner:
  `mode1_keep_fullworld_ep_cache=0` or single-live / disable-MC2-cache enabled
