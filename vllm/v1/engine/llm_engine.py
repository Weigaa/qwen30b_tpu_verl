...
class LLMEngine:
    ...
    def _staged_shrink_target(self, natural_active_ranks: list[int],
        ...
        current = (list(self.elastic_ep_active_ranks)
                   if self.elastic_ep_active_ranks is not None
                   else list(range(dp_world_size)))
        decision = decide_staged_shrink(
            ...
            unfinished_ranks=natural_active_ranks,
            role_plan=plan,
        ...
        if decision.should_shrink:
            logger.info(
                "Shrink-aware staged trigger: stage=%s current=%s unfinished=%s target=%s",
