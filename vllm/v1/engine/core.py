...
class EngineCore:
    ...
    def __init__(
        ...
        # Keep a local running-state flag even for the in-process EngineCore
        # path so front-end logic can safely query whether cross-rank DP work is
        # still in flight.
        ...
        self._base_step_fn = self.step_fn
        ...
        self.dp_group = None
        self._owns_dp_group = False
        if self.use_dp_inproc:
            self.step_counter = 0
    ...
    def _initialize_kv_caches(
        ...
        # Keep the per-worker KV cache configs so RL rollout can explicitly
        # tear down and rebuild KV cache between trainer / inference phases
        # without relying on generic sleep-mode allocator remapping.
    ...
    def step_with_dp(self) -> tuple[dict[int, EngineCoreOutputs] | None, bool]:
        outputs, model_executed = self._base_step_fn()
        self.post_step(model_executed=model_executed)
        local_unfinished_reqs = self.scheduler.has_unfinished_requests()
        if not model_executed:
            if not local_unfinished_reqs and not self.engines_running:
                return outputs, False
            self.execute_dummy_batch()
        dp_group = self._ensure_inproc_dp_group()
        self.engines_running = ParallelConfig.has_unfinished_dp(
            dp_group, local_unfinished_reqs
        )
        return outputs, model_executed

    def _ensure_inproc_dp_group(self):
    ...
    def shutdown(self):
        ...
        if self._owns_dp_group and self.dp_group is not None:
