...
def _maybe_cap_custom_mode1_parity_num_blocks(
        vllm_config: VllmConfig, num_blocks: int,
        min_block_size: int, num_kv_groups: int) -> int:
    """Restore the old custom mode=1 native-KV cap when explicitly enabled.

    Old v11 mode=1 floor8 zero-headroom runs capped startup KV to 377,344
    tokens (2948 blocks) so custom mode1 matched native startup budget exactly.
    Current code otherwise profiles slightly higher (e.g. 377,856 tokens,
    2952 blocks), which is ~512 extra tokens and can be enough to trip the
    tight first resume(kv_cache) allocation.
    """
    raw_cap = os.getenv("VLLM_ASCEND_MODE1_PARITY_MAX_KV_TOKENS", "")
    if not raw_cap:
        return num_blocks
    try:
        max_tokens = int(raw_cap)
    except ValueError:
        logger.warning(
            "Ignoring invalid VLLM_ASCEND_MODE1_PARITY_MAX_KV_TOKENS=%r",
            raw_cap)
        return num_blocks
    if max_tokens <= 0 or min_block_size <= 0 or num_kv_groups <= 0:
        return num_blocks

    capped_blocks = (max_tokens // min_block_size) * num_kv_groups
    if capped_blocks <= 0 or capped_blocks >= num_blocks:
        return num_blocks

    logger.info(
        "Capping custom mode1 parity KV blocks: requested_blocks=%d "
        "capped_blocks=%d min_block_size=%d kv_groups=%d capped_tokens=%d",
        num_blocks, capped_blocks, min_block_size, num_kv_groups,
        (capped_blocks // num_kv_groups) * min_block_size)
...
def get_kv_cache_config_from_groups(vllm_config: VllmConfig,
    ...
    if len(kv_cache_groups) == 1 and \
        ...
        num_blocks = _maybe_cap_custom_mode1_parity_num_blocks(
            vllm_config, num_blocks, min_block_size, len(kv_cache_groups))
        kv_cache_tensors = []
