...
usage() {
    cat <<'EOF'
Usage:
  ./run_mode5_perf_clean_test.sh [floor]
  ./run_mode5_perf_clean_test.sh --floor 1

Environment overrides:
  MODE5_FLOOR=1
  REPO_ROOT=/path/to/log/output/root
  PATCH_TREE=/path/to/code/tree
  HCCL_IF_BASE_PORT=47141
  MASTER_PORT=26140
  VERL_HCCL_IF_BASE_PORT_START=47141

Examples:
  ./run_mode5_perf_clean_test.sh
  ./run_mode5_perf_clean_test.sh 1
  ./run_mode5_perf_clean_test.sh --floor 4
EOF
}
...
case "$floor" in
    1|2|4|8|16) ;;
    *)
        echo "unsupported mode=5 floor: $floor; expected one of 1,2,4,8,16" >&2
        exit 2
        ;;
esac
...
tee_log="$REPO_ROOT/mode5_floor${floor}_perf_clean_${stamp}.log"
...
export VLLM_ASCEND_MODE4_RUNTIME_MIN_COMPUTE_GROUP_SIZE="${VLLM_ASCEND_MODE4_RUNTIME_MIN_COMPUTE_GROUP_SIZE:-$floor}"
export VLLM_ASCEND_MODE5_RUNTIME_MIN_COMPUTE_GROUP_SIZE="${VLLM_ASCEND_MODE5_RUNTIME_MIN_COMPUTE_GROUP_SIZE:-$floor}"
...
export HCCL_IF_BASE_PORT="${HCCL_IF_BASE_PORT:-47141}"
export MASTER_PORT="${MASTER_PORT:-26140}"
export VERL_HCCL_IF_BASE_PORT_START="${VERL_HCCL_IF_BASE_PORT_START:-47141}"
...
' "$tee_log"
printf '[mode5 perf clean] floor=%s mode4_runtime_floor=%s mode5_runtime_floor=%s\n' "$floor" "$VLLM_ASCEND_MODE4_RUNTIME_MIN_COMPUTE_GROUP_SIZE" "$VLLM_ASCEND_MODE5_RUNTIME_MIN_COMPUTE_GROUP_SIZE"
