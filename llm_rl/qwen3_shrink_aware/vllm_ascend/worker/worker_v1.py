...
class NPUWorker(WorkerBase):
    ...
    def _mode1_fixed_topology_group_reuse_enabled(self) -> bool:
        return (self._is_mode1_low_floor_elastic_runtime()
                and _env_flag(
                    "VLLM_ASCEND_MODE1_PARITY_FIXED_TOPOLOGY_REUSE", "0"))
    ...
    def _should_keep_stale_group_cache_for_custom_mode1_parity(
        ...
        if not stale_group_ranks:
            return False
        if self._is_mode1_fixed_topology_reusable_group(
    ...
    def _drop_stale_cached_elastic_parallel_groups(
        ...
        if dropped_groups > 0 or dropped_signatures > 0:
            logger.info(
                "Elastic parallel stale group cache dropped: rank=%s keep_ranks=%s dropped_groups=%s dropped_signatures=%s deferred_groups=%s cleanup_ms=%.2f",
                self.rank, keep_group_ranks, dropped_groups,
                dropped_signatures, deferred_groups, cleanup_ms)
        return {
            "total_ms": (time.perf_counter() - drop_start_t) * 1000.0,
            "cleanup_ms": cleanup_ms,
            "cleanup_gc_ms": cleanup_gc_ms,
    ...
    def _warmup_post_shrink_moe_dispatch(self,
        ...
        if mode1_low_floor_runtime and force_mode1_parity_warmup and not hasattr(
            ...
            logger.info(
                ...
                "active_ranks=%s reason=mode1_parity_no_helper",
                self.rank,
        ...
        if mode1_low_floor_runtime and force_mode1_parity_warmup:
            ...
            try:
                ...
                logger.exception(
                    ...
                    "active_ranks=%s reason=mode1_parity_moe_only "
                    "tokens=%s",
                    self.rank,
                    list(active_signature),
                    warmup_tokens,
            ...
            logger.info(
                ...
                "active_ranks=%s reason=mode1_parity_moe_only tokens=%s "
                "total_ms=%.2f",
                self.rank,
                list(active_signature),
                warmup_tokens,
