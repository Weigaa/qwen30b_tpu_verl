...
usage() {
    cat <<'EOF'
Usage:
  ./run_mode1_local_length_sorted_e2e_floor4.sh [extra hydra args...]

Mode=1 floor=4 length-sorted end-to-end rollout-time experiment:
  ...
  4. Runs mode=1 floor=4 with the exact rank plan, without requiring staged
     donor/wave/final shrink behavior.
EOF
}
...
export OUTPUT_SUBDIR="${OUTPUT_SUBDIR:-mode1_length_sorted_e2e_floor4}"
...
PLAN_DIR="${PLAN_DIR:-$REPO_ROOT/mode1_length_sorted_e2e_floor4/oracle}"
...
tee_log="$REPO_ROOT/mode1_local_length_sorted_e2e_floor4_${stamp}.log"
...
printf '[mode1 length-sorted e2e] mode=1 floor=4 util=%s kv_cap_floor4=%s total_epochs=%s\n' \
    "$ROLLOUT_GPU_MEMORY_UTILIZATION" \
    "$VLLM_ASCEND_MODE1_PARITY_MAX_KV_TOKENS_FLOOR4" \
    "$TRAINER_TOTAL_EPOCHS"
printf '[mode1 length-sorted e2e] max_rank_peak_tokens=%s active_peak_safety_factor=%s max_response_len=%s shrink_not_required=true\n' \
    "$MAX_RANK_PEAK_TOKENS" "$ACTIVE_PEAK_SAFETY_FACTOR" "$MAX_RESPONSE_LEN"
