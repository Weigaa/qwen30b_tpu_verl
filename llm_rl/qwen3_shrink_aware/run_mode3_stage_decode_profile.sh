...
profile_root="${PROFILE_ROOT:-$REPO_ROOT/mode3_stage_decode_profiles/$stamp}"
tee_log="$REPO_ROOT/mode3_stage_decode_profile_${stamp}.log"
...
export HCCL_IF_BASE_PORT="${HCCL_IF_BASE_PORT:-46941}"
export MASTER_PORT="${MASTER_PORT:-25940}"
export VERL_HCCL_IF_BASE_PORT_START="${VERL_HCCL_IF_BASE_PORT_START:-46941}"
...
echo "[mode3 stage decode profile] runtime_cwd=$PATCH_TREE"
echo "[mode3 stage decode profile] tee_log=$tee_log"
echo "[mode3 stage decode profile] profile_root=$profile_root"
echo "[mode3 stage decode profile] op_profile_dir=$VLLM_ASCEND_BUCKET_OP_PROFILE_DIR"
echo "[mode3 stage decode profile] stages=$VLLM_ASCEND_BUCKET_OP_PROFILE_STAGES samples=$VLLM_ASCEND_BUCKET_OP_PROFILE_STAGE_SAMPLES ranks=$VLLM_ASCEND_BUCKET_OP_PROFILE_RANKS contents=$VLLM_ASCEND_BUCKET_OP_PROFILE_CONTENTS"
echo "[mode3 stage decode profile] ports HCCL_IF_BASE_PORT=$HCCL_IF_BASE_PORT MASTER_PORT=$MASTER_PORT VERL_HCCL_IF_BASE_PORT_START=$VERL_HCCL_IF_BASE_PORT_START"
