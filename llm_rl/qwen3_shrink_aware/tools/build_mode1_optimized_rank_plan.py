...
def _read_baseline_stats(baseline_dir: Path, steps: int,
    ...
    for step in range(1, steps + 1):
        ...
        for row, length in zip(rows, lengths, strict=True):
            by_input[str(row["input"])].append(float(length))
        for prompt_input, values in by_input.items():
            if len(values) != responses_per_prompt:
                raise RuntimeError(
                    f"step={step} expected {responses_per_prompt} responses per "
            ...
            stats[prompt_input] = PromptStats(
                source_idx=-1,
                mean=float(arr.mean()),
...
def _map_stats_to_dataset(train_file: Path,
                          ...
                          max_samples: int,
                          min_samples: int) -> tuple[pd.DataFrame, list[PromptStats]]:
    ...
    selected = df.iloc[:sample_size].copy()

    rows: list[tuple[int, PromptStats]] = []
    missing: list[str] = []
    for source_idx, row in selected.iterrows():
        prompt_input = _prompt_to_input(row["prompt"])
