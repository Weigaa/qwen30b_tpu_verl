...
class AscendUnquantizedFusedMoEMethod(UnquantizedFusedMoEMethod):
    ...
    def process_weights_after_loading(self, layer):
        ...
        # Keep the Parameter objects stable across RL weight reloads. Rebinding
        # them drops weight_loader attrs and can move post-load allocations out
        # of vLLM-Ascend's sleep-mode memory pool.
...
class AscendFusedMoE(FusedMoE):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.moe_config.use_ep:
            ...
            # Respect vLLM's non-EP FusedMoE semantics.  vLLM still creates an
            # EP process group for MoE models, but when enable_expert_parallel
            # is false the parent layer expects every TP/DP shard to expose the
            # full logical expert set with expert_map=None.
            self.global_expert_map = None
            self._expert_map = None
            self.log2phy = None
            self.global_redundant_expert_num = 0
        self.global_num_experts = num_experts + self.global_redundant_expert_num
        self.dynamic_eplb = (self.moe_config.use_ep and eplb_config.dynamic_eplb
