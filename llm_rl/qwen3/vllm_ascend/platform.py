...
class NPUPlatform(Platform):
    ...
    def check_and_update_config(cls, vllm_config: VllmConfig) -> None:
        ...
        if ascend_compilation_config:
            ascend_compilation_options = vllm_config.additional_config.setdefault("ascend_compilation_config", {})
            ascend_compilation_options.update(
            ...
            if ascend_compilation_options.get("fuse_allreduce_rms", True):
        ...
        eager_compile = os.getenv("VLLM_ASCEND_EAGER_COMPILE", "0").lower() in (
            "1",
            "true",
            "yes",
            "on",
        )
        force_cudagraph_none = os.getenv("VLLM_ASCEND_FORCE_CUDAGRAPH_NONE", "0").lower() in (
            "1",
            "true",
            "yes",
            "on",
        )
        force_compile_without_aclgraph = os.getenv(
            "VLLM_ASCEND_FORCE_COMPILE_WITHOUT_ACLGRAPH", "0"
        ).lower() in (
            "1",
            "true",
            "yes",
        ...
        if enforce_eager and not eager_compile:
            logger.info("Compilation disabled, using eager mode by default")
            compilation_config.mode = CompilationMode.NONE
            if compilation_config.splitting_ops is None:
                compilation_config.splitting_ops = []
        elif enforce_eager and eager_compile:
            logger.info(
                "VLLM_ASCEND_EAGER_COMPILE=1: keep VLLM_COMPILE while "
                "cudagraph remains disabled for eager rollout."
            )
        ...
        if force_cudagraph_none:
            logger.warning(
                "VLLM_ASCEND_FORCE_CUDAGRAPH_NONE=1: forcing cudagraph_mode=NONE "
                "for graph correctness isolation."
            )
            compilation_config.cudagraph_mode = CUDAGraphMode.NONE
            # Keep the no-ACLGraph compile path semantically aligned with
            # enforce_eager=True + VLLM_ASCEND_EAGER_COMPILE=1.  Leaving the
            # non-eager piecewise defaults in place (attention splitting,
            # capture sizes, graph-only passes) can change qwen3 MoE rollout
            # logits/stop behavior even though replay itself is disabled.
            compilation_config.splitting_ops = []
            compilation_config.cudagraph_capture_sizes = []
            compilation_config.max_cudagraph_capture_size = 0
            compilation_config.cudagraph_copy_inputs = False
            if pass_config is not None:
                pass_config.eliminate_noops = False
                pass_config.fuse_norm_quant = False
                pass_config.fuse_act_quant = False
                ...
                if hasattr(pass_config, "enable_qk_norm_rope_fusion"):
                    pass_config.enable_qk_norm_rope_fusion = False
        ...
        if compilation_config.cudagraph_mode == CUDAGraphMode.NONE:
            if eager_compile or force_compile_without_aclgraph:
                compilation_config.mode = CompilationMode.VLLM_COMPILE
            else:
                compilation_config.mode = CompilationMode.NONE
            ...
            if os.getenv("VLLM_ASCEND_PIECEWISE_COPY_INPUTS", "0").lower() in (
                "1",
                "true",
                "yes",
                "on",
            ):
                # Optional isolation knob for dynamic-input graph replay.
                # Default stays aligned with upstream/ref_ziyi, where PIECEWISE
                # uses direct graph inputs and cudagraph_copy_inputs=False.
        ...
        if ascend_config.SLO_limits_for_dynamic_batch != -1:
            ...
            if not vllm_config.scheduler_config.enable_chunked_prefill:
                logger.warning(
                    "Dynamic batch scheduler is enabled with chunked prefill disabled. "
                    "Honoring the explicit rollout setting."
                )
        ...
        if os.path.exists(CUSTOM_OPP_PATH):
            ...
            if use_local_op_api and os.path.exists(custom_op_api_lib):
                ...
                if current_ld_library_path:
                    ...
                    os.environ["LD_LIBRARY_PATH"] = custom_op_api_lib
        _CUSTOM_OP_REGISTERED = True
