...
def _sync_shrink_aware_env_from_meta(meta_info: dict[str, Any]) -> None:
    """Install the driver-selected AdaFloor plan in each rollout process."""
    ...
    os.environ["VLLM_ASCEND_SHRINK_AWARE_TARGET_POLICY"] = str(
        runtime.get("target_policy", "natural"))
    ...
    os.environ["VLLM_ASCEND_SHRINK_AWARE_DRY_RUN"] = "0"
...
class vLLMRollout(BaseRollout):
    def __init__(
        ...
        if not config.enforce_eager and cudagraph_capture_sizes:
            requested_cudagraph_mode = config.get("cudagraph_mode") or "FULL"
            if not isinstance(requested_cudagraph_mode, str):
                raise TypeError(
                    "actor_rollout_ref.rollout.cudagraph_mode must be a string, "
                    f"got {type(requested_cudagraph_mode).__name__}"
                )
            requested_cudagraph_mode = requested_cudagraph_mode.upper()
            ...
            if requested_cudagraph_mode not in supported_cudagraph_modes:
                choices = ", ".join(sorted(supported_cudagraph_modes))
                raise ValueError(
                    "actor_rollout_ref.rollout.cudagraph_mode must be one of "
                    f"{choices}, got {requested_cudagraph_mode!r}"
                )
            torch._dynamo.config.log_compilation_metrics = False
            compilation_config["compilation_config"] = {
                "cudagraph_capture_sizes": cudagraph_capture_sizes,
                "cudagraph_mode": requested_cudagraph_mode,
            }
            logger.info(
