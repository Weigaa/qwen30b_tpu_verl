...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        self.rollout_shrink_aware_short_step_cap_enable = os.getenv(
            "VLLM_ROLLOUT_SHRINK_AWARE_SHORT_STEP_CAP_ENABLE", "0"
        ).lower() in ("1", "true", "yes", "on")
        self.rollout_shrink_aware_short_step_exit_threshold = float(
            os.getenv("VLLM_ROLLOUT_SHRINK_AWARE_SHORT_STEP_EXIT_THRESHOLD", "4096")
        )
        self.rollout_shrink_aware_short_step_cap_tokens = int(
            os.getenv("VLLM_ROLLOUT_SHRINK_AWARE_SHORT_STEP_CAP_TOKENS", "4096")
        )
        floor_values = os.getenv(
            "VLLM_ROLLOUT_SHRINK_AWARE_SHORT_STEP_CAP_FLOORS", "4"
        )
        self.rollout_shrink_aware_short_step_cap_floors = {
            int(item.strip()) for item in floor_values.split(",") if item.strip()
        }
    ...
    def _maybe_compute_shrink_aware_short_step_response_cap(
        ...
        if plan_cap is not None:
            ...
            logger.info(
                "AdaFloor TailGuard response cap: selected_floor=%s "
                "plan_cap=%s predicted_step_exit=%s sampling_override=%s",
                selected_floor,
                cap,
                kv_plan.get("tail_guard_predicted_step_exit"),
                cap < hard_cap,
            )
            return None if cap >= hard_cap else cap
        ...
        if predicted_load is None:
            return None
        try:
            predicted_step_exit = float(
                np.max(np.asarray(predicted_load, dtype=np.float64))
            )
        except (TypeError, ValueError):
            return None
        ...
        cap = max(
            1,
            min(self.rollout_shrink_aware_short_step_cap_tokens, hard_cap),
        )
        if cap >= hard_cap:
            return None
        logger.info(
            "AdaFloor short-step response cap: selected_floor=%s "
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                if shrink_aware_result.enabled:
                    if self.async_rollout_mode:
                        raise RuntimeError(
                            "AdaFloor eager migration currently supports only "
                            "synchronous vLLM rollout; set "
                            "actor_rollout_ref.rollout.async_scheduling=false")
                    role_plan = shrink_aware_result.role_plan
                    metrics["rollout/shrink_aware_enabled"] = 1
                    metrics["rollout/shrink_aware_package_locality_score"] = (
                        role_plan.package_locality_score)
                    for rank, load in (
                        shrink_aware_result.assignment_plan.
                        per_rank_predicted_load.items()
                    ):
                        metrics[
                            f"rollout/shrink_aware_rank_{rank}_predicted_load"
                        ] = load
                ...
                with marked_timer("step", timing_raw):
                    ...
                    if self.config.algorithm.adv_estimator == AdvantageEstimator.REMAX:
                        ...
                        with marked_timer("gen_max", timing_raw, color="purple"):
                            ...
                            if spread_repeat_restore_indices is not None:
                                gen_baseline_output.reorder(
                                    spread_repeat_restore_indices)
                            elif length_balance_restore_indices is not None:
                                gen_baseline_output.reorder(
                                    length_balance_restore_indices)
