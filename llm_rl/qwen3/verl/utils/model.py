...
def load_mcore_dist_weights(
    ...
    def prune_unexpected_extra_state_objects(sharded_state_dict):
        ...
        unexpected_extra, pruned_state_dict = extract_matching_values(
            sharded_state_dict, is_unexpected_extra_state
        )
        removed_keys = sorted(
            {x.key for x in nested_values(unexpected_extra) if isinstance(x, ShardedObject)}
        )
        if removed_keys:
            print(
    ...
    # `ref` can be loaded in a more permissive mode to avoid distributed
    # checkpoint mismatch validation on fragile runtime stacks.
    strict = (
    ...
    for model_idx, model in enumerate(parallel_model):
        ...
        if os.getenv('USE_ALLTOALL_OVERLAP', '0') == '1':
            new_ssd = dist_checkpointing.load(
                ssd,
                dist_weight_path,
