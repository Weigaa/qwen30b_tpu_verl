...
OUTPUT_ROOT="${OUTPUT_ROOT:-/data/adafloor_shared_state/qwen3_aclgraph_full_decode_parity_20260812}"
...
mkdir -p "$RUN_ROOT"
{
    ...
    echo "attention_graph=$([[ "$MODE" == graph ]] && echo true || echo false)"
    echo "moe_graph=$([[ "$MODE" == graph ]] && echo true || echo false)"
    echo "cudagraph_mode=$([[ "$MODE" == graph ]] && echo FULL_DECODE_ONLY || echo NONE)"
    echo "graph_ex=false"
    echo "torchair_ge=false"
} > "$RUN_ROOT/protocol.env"

contract_files=(
    "$SCRIPT_DIR/run_qwen3_aclgraph_full_decode_parity.sh"
