...
_RAY_WORKER_PASSTHROUGH_PREFIXES = (
    ...
    "VLLM_ROLLOUT_SLEEP_LEVEL",
    "VLLM_ROLLOUT_REUSE_ACLGRAPH_AFTER_WEIGHT_UPDATE",
    "VLLM_ROLLOUT_CAMEM_WEIGHT_RELOAD",
    "VLLM_ROLLOUT_FILTER_EMPTY_WEIGHT_SHARDS",
    "VLLM_ENABLE_MC2",
    "ASCEND_CUSTOM_OPP_PATH",
    "MEGATRON_CI_DISABLE_EXPANDABLE_SEGMENTS",
    "PYTORCH_NPU_ALLOC_CONF",
    "VERL_MEGATRON_EAGER_WEIGHT_SYNC_GROUP_INIT",
...
def _ray_worker_passthrough_env() -> dict[str, str]:
    return {
        key: value
        for key, value in os.environ.items()
        if key.startswith(_RAY_WORKER_PASSTHROUGH_PREFIXES)
    }
...
_HCCL_IF_BASE_PORT_BLOCK = _env_int("VERL_HCCL_IF_BASE_PORT_BLOCK", 16384)
...
_HCCL_PHASE_PORT_OFFSETS = (0, 4096, 8192, 12288)
_HCCL_PORTS_PER_PHASE = 32
...
def _alloc_hccl_if_base_port() -> str:
    ...
    with _HCCL_IF_BASE_PORT_LOCK:
        for _ in range(512):
            ...
            if all(
                _can_bind_port(port + offset + port_index)
                for offset in _HCCL_PHASE_PORT_OFFSETS
                for port_index in range(_HCCL_PORTS_PER_PHASE)
            ):
                return str(port)
    raise RuntimeError(
...
def _alloc_master_port(hccl_if_base_port: str) -> str:
    ...
    with _MASTER_PORT_LOCK:
        for _ in range(512):
            ...
            if _ranges_overlap(port, _MASTER_PORT_BLOCK, hccl_port,
                               _HCCL_IF_BASE_PORT_BLOCK):
                continue
    ...
    def _init_with_resource_pool(self, resource_pool, ray_cls_with_init, bin_pack, detached, worker_env=None):
        ...
        for pg_idx, pg in enumerate(sort_placement_group_by_node_ip(pgs)):
            ...
            for local_rank in range(local_world_size):
                ...
                for key, value in _ray_worker_passthrough_env().items():
