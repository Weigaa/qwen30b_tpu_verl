...
def _hf_to_mcore_config_deepseek(
    hf_config: PretrainedConfig,
    dtype: torch.dtype,
    *,
    v3_router_defaults: bool,
    **override_transformer_config_kwargs,
    ...
    topk_method = getattr(hf_config, "topk_method", "noaux_tc" if v3_router_defaults else "greedy")
    scoring_func = getattr(hf_config, "scoring_func", "sigmoid" if v3_router_defaults else "softmax")
    enable_expert_bias = topk_method == "noaux_tc"
    if enable_expert_bias and scoring_func != "sigmoid":
        raise ValueError(
            "DeepSeek aux-loss-free expert bias requires sigmoid routing, "
            f"got topk_method={topk_method!r} scoring_func={scoring_func!r}."
        )
    ...
    args: dict = _get_mla_transformer_config(
        ...
        # mcore 0.12 moe
        moe_router_dtype="fp64" if v3_router_defaults else None,
...
def hf_to_mcore_config_dpskv2(
    hf_config: PretrainedConfig, dtype: torch.dtype, **override_transformer_config_kwargs
) -> MLATransformerConfig:
    """Convert DeepSeek-V2 configs without importing V3 router semantics."""

    return _hf_to_mcore_config_deepseek(
        hf_config,
        dtype,
        v3_router_defaults=False,
        **override_transformer_config_kwargs,
    )
