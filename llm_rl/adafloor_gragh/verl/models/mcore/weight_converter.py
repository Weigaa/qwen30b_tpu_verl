    ...
    def _convert_mlp_param(self, name: str, params: list[torch.Tensor]) -> tuple[list[str], list[torch.Tensor]]:
        ...
        if name_after_layer in name_map_after_layer:
            ...
            if "mlp.experts.weight1" in name:
                if len(params) % 2 != 0:
                    raise ValueError(
                        "DeepSeek grouped expert weight1 must expand to "
                        f"gate/up pairs, got {len(params)} tensors"
                    )
                num_moe_experts = len(params) // 2
                for expert_id in range(num_moe_experts):
                ...
                for expert_id in range(len(params)):
                    convert_names.append(
...
# V2 and V3 share MLA, shared-expert, and routed-expert tensor names. V2
# simply omits the optional router expert bias, so conversion never emits an
# e_score_correction_bias tensor for that checkpoint.
McoreToHFWeightConverterDpskv2 = McoreToHFWeightConverterDpskv3
