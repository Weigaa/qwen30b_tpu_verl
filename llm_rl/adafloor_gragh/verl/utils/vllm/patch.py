        ...
        try:
            ...
            if hasattr(loaded_weight, "numel"):
                ...
                try:
                    ...
                    pass
            shard_id = (args[3] if len(args) > 3 else kwargs.get(
...
def _iter_vllm_moe_experts(model):
...
def patch_vllm_moe_model_weight_loader(model):
    """Attach the fused-MoE loaders and begin one online reload."""
    for mlp, experts in _iter_vllm_moe_experts(model):
        prepare_reload = getattr(experts,
                                 "prepare_online_expert_weight_reload", None)
...
def finalize_vllm_moe_model_weight_loader(model):
    """Fail fast when an online fused-MoE reload left a partial weight."""
    for _, experts in _iter_vllm_moe_experts(model):
        finalize_reload = getattr(experts,
                                  "finalize_online_expert_weight_reload", None)
        if callable(finalize_reload):
            finalize_reload()


def abort_vllm_moe_model_weight_loader(model):
    """Drop transient paired-shard state after a failed online reload."""
    for _, experts in _iter_vllm_moe_experts(model):
        abort_reload = getattr(experts,
                               "abort_online_expert_weight_reload", None)
        if callable(abort_reload):
            abort_reload()
