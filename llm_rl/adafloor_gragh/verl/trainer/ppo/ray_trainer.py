...
from verl.experimental.dataset.shrink_aware_assignment import (
    maybe_apply_shrink_aware_schedule,
...
def _export_shrink_aware_role_plan(role_plan, shrink_cfg) -> None:
    if role_plan is None:
        return
    os.environ["VLLM_ASCEND_SHRINK_AWARE_ENABLE"] = "1"
    os.environ["VLLM_ASCEND_SHRINK_AWARE_MODE"] = str(
        _shrink_cfg_get(shrink_cfg, "shrink_aware_mode", "staged")).lower()
...
def _shrink_cfg_get(cfg, key, default=None):
    if cfg is None:
        return default
    if hasattr(cfg, "get"):
...
class RayPPOTrainer:
    ...
    def _restore_rollout_elastic_parallel_groups_if_needed(self) -> None:
        ...
        if restore_fn is None:
            return
        if os.getenv("VERL_SIDECAR_ENABLE", "0").lower() in (
            "1", "true", "yes", "on"
        ):
            stop_start = time.perf_counter()
            ack = request_sidecar_stop_before_restore()
            logger.info(
                "Sidecar pre-restore release acknowledged: request_id=%s "
                "lease_id=%s elapsed_s=%.3f",
                ack["request_id"],
                ack["lease_id"],
                time.perf_counter() - stop_start,
            )
        start = time.perf_counter()
        logger.info(
            "Mode1 step timeline: driver_restore_rpc_start "
            "driver_restore_rpc_start_time=%.9f",
            time.time(),
        )
        restore_fn()
        logger.info(
    ...
    def _log_rollout_data(
        ...
        with marked_timer("dump_rollout_generations", timing_raw, color="green"):
            ...
            for field in (
                ...
                "fixed_work_replay_row_ordinal",
                "fixed_work_replay_source_row_ordinal",
                "fixed_work_replay_source_step",
                "fixed_work_replay_source_length",
                "fixed_work_replay_target_length",
                "fixed_work_replay_trace_sha256",
                "decoded_response_length",
    ...
    def _run_rollout_decode_batchsize_benchmark(self) -> dict:
        ...
        for local_step in range(total_steps):
            ...
            response_tokens = gen_batch_output.batch["responses"].detach().cpu()
            response_mask_cpu = response_mask.detach().cpu()
            response_digest = hashlib.sha256()
            response_row_digests = []
            response_row_token_ids = []
            for response_row, mask_row in zip(response_tokens, response_mask_cpu, strict=True):
                valid_tokens = int(mask_row.sum().item())
                valid_response = response_row[:valid_tokens].contiguous()
                length_bytes = valid_tokens.to_bytes(8, "little", signed=False)
                token_bytes = valid_response.numpy().tobytes()
                response_digest.update(length_bytes)
                response_digest.update(token_bytes)
                row_digest = hashlib.sha256()
                row_digest.update(length_bytes)
                row_digest.update(token_bytes)
                response_row_digests.append(row_digest.hexdigest())
                response_row_token_ids.append(valid_response.tolist())
            prompt_tokens = gen_batch_output.batch["prompts"].detach().cpu().contiguous()
            prompt_digest = hashlib.sha256()
            prompt_digest.update(json.dumps(list(prompt_tokens.shape)).encode("ascii"))
            prompt_digest.update(prompt_tokens.numpy().tobytes())
            ...
            record = {
                ...
                "response_token_sha256": response_digest.hexdigest(),
                "response_row_token_sha256": response_row_digests,
                "response_row_token_ids": response_row_token_ids,
                "prompt_token_sha256": prompt_digest.hexdigest(),
                "wall_s": wall_s,
        ...
        summary = {
            ...
            "measured_response_token_sha256": [
                record["response_token_sha256"] for record in measured
            ],
            "measured_response_row_token_sha256": [
                record["response_row_token_sha256"] for record in measured
            ],
            "measured_prompt_token_sha256": [
                record["prompt_token_sha256"] for record in measured
            ],
        }
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            ...
            for batch_dict in self.train_dataloader:
                ...
                fixed_work_enabled = bool(os.getenv("VERL_FIXED_WORK_REPLAY_TRACE"))
                paired_sampling_enabled = os.getenv(
                    "VERL_PAIRED_REQUEST_SAMPLING_SEEDS", "0"
                ).lower() in ("1", "true", "yes", "on")
                if fixed_work_enabled or paired_sampling_enabled:
                    occurrence_source = gen_batch.non_tensor_batch.get("index")
                    if occurrence_source is None:
                        raise RuntimeError(
                            "paired rollout requires the dataset source occurrence "
                            "in extra_info.index"
                        )
                    try:
                        prompt_occurrences = validate_prompt_occurrence_ordinals(
                            occurrence_source,
                            int(self.config.actor_rollout_ref.rollout.n),
                        )
                    except ValueError:
                        if fixed_work_enabled:
                            raise
                        physical_rows = gen_batch.non_tensor_batch.get(
                            "dataset_item_idx"
                        )
                ...
                if fixed_work_enabled:
                    gen_batch.non_tensor_batch["fixed_work_replay_row_ordinal"] = (
                        np.arange(len(gen_batch), dtype=np.int64)
                    )
                if paired_sampling_enabled:
                    gen_batch.non_tensor_batch["rollout_sample_index"] = (
                        build_rollout_sample_indices(
                            prompt_count=len(batch.batch),
                ...
                fixed_work_identity_snapshot = None
                if fixed_work_enabled:
                    fixed_work_identity_snapshot = (
                        np.asarray(
                            gen_batch.non_tensor_batch[
                                "prompt_occurrence_ordinal"
                            ],
                ...
                if shrink_aware_result.enabled:
                    shrink_cfg = self.config.actor_rollout_ref.rollout.get(
                        "shrink_aware", {})
                    _export_shrink_aware_role_plan(
                        shrink_aware_result.role_plan, shrink_cfg)
                    metrics["rollout/shrink_aware_enabled"] = 1
                    metrics["rollout/shrink_aware_package_locality_score"] = (
                        shrink_aware_result.role_plan.package_locality_score)
                    for rank, load in (
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("gen", timing_raw, color="red"):
                        ...
                        restore_shrink_aware_order(
                            gen_batch_output, shrink_aware_result)
                        if data_rebalance:
                            gen_batch_output.reorder(interleave_indices)
                        if fixed_work_identity_snapshot is not None:
                            validate_fixed_work_restored_order(
                                fixed_work_identity_snapshot[0],
                                fixed_work_identity_snapshot[1],
                    ...
                    if self.config.algorithm.adv_estimator == AdvantageEstimator.REMAX:
                        ...
                        with marked_timer("gen_max", timing_raw, color="purple"):
                            ...
                            restore_shrink_aware_order(
                                gen_baseline_output, shrink_aware_result)
                            if data_rebalance:
