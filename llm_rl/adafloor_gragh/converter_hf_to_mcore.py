...
@torch.inference_mode()
def convert_checkpoint_from_transformers_to_megatron_deepseek(
    hf_model,
    model,
    hf_config,
    tfconfig,
    ...
    for layer_idx, (layer, hf_layer) in enumerate(
        ...
        if not hasattr(layer.mlp, "router"):
            ...
            numel += safe_copy(hf_layer.mlp.gate.weight, layer.mlp.router.weight)
            hf_expert_bias = getattr(hf_layer.mlp.gate, "e_score_correction_bias", None)
            mcore_expert_bias = getattr(layer.mlp.router, "expert_bias", None)
            if (hf_expert_bias is None) != (mcore_expert_bias is None):
                raise ValueError(
                    "DeepSeek router-bias mismatch between HuggingFace and Megatron "
                    f"at layer {global_layer_idx}: hf={hf_expert_bias is not None} "
                    f"mcore={mcore_expert_bias is not None}"
                )
            if hf_expert_bias is not None:
                # MCore initializes this buffer in the model dtype and restores
                # fp32 on first forward, so dtype equality is intentionally not
                # required during checkpoint conversion.
                numel += safe_copy(
                    hf_expert_bias, mcore_expert_bias, skip_dtype_assert=True
                )
            num_experts = len(hf_layer.mlp.experts)
            if num_experts % ep_size:
                raise ValueError(
                    f"routed experts {num_experts} are not divisible by EP size {ep_size}"
                )
            num_local_experts = num_experts // ep_size
            expert_idx_start = ep_rank * num_local_experts
            expert_idx_end = (ep_rank + 1) * num_local_experts
            if tfconfig.moe_grouped_gemm:
                for i, hf_expert in enumerate(hf_layer.mlp.experts):
                    ...
                    if hasattr(experts, "weight1") and hasattr(experts, "weight2"):
                        hidden_size = hf_expert.gate_proj.weight.shape[1]
                        weight1 = experts.weight1.view(
                            num_local_experts, hidden_size, -1
                        )
                        weight2 = experts.weight2.view(
                            num_local_experts, -1, hidden_size
                        )
                        fc1_weight = torch.cat(
                            [
                                hf_expert.gate_proj.weight.transpose(0, 1).contiguous(),
                        ...
                        fc2_weight = hf_expert.down_proj.weight.transpose(0, 1).contiguous()
                        numel += safe_copy(fc1_weight, weight1[local_expert_idx])
                        numel += safe_copy(fc2_weight, weight2[local_expert_idx])
                    else:
                        fc1_weight = torch.cat(
                            [hf_expert.gate_proj.weight, hf_expert.up_proj.weight]
                        )
                        linear_fc1_weight = getattr(
                            experts.linear_fc1, f"weight{local_expert_idx}"
                        )
                        linear_fc2_weight = getattr(
                            experts.linear_fc2, f"weight{local_expert_idx}"
                        )
                        numel += safe_copy(fc1_weight, linear_fc1_weight)
                        numel += safe_copy(
                            hf_expert.down_proj.weight, linear_fc2_weight
                        )
            else:
                for i, hf_expert in enumerate(hf_layer.mlp.experts):
                    if i < expert_idx_start or i >= expert_idx_end:
                        continue
                    local_expert_idx = i - expert_idx_start
                    expert = layer.mlp.experts.local_experts[local_expert_idx]
                    fc1_weight = torch.cat([hf_expert.gate_proj.weight, hf_expert.up_proj.weight])
        ...
        if hasattr(layer.mlp, "router"):
            expert_numel = sum(
                tensor.numel()
                for tensor in hf_layer.mlp.experts[0].state_dict().values()
            )
            numel_hf_one_layer -= expert_numel * (num_experts - num_local_experts)
...
def support_distributed_convert(hf_config: AutoConfig) -> bool:
    for arch in ["DeepseekV2ForCausalLM", "DeepseekV3ForCausalLM", "Qwen3MoeForCausalLM", "Qwen2MoeForCausalLM"]:
...
def convert_hf_to_mcore(
    ...
    if repatch:
        config_repatch = {}
        if hf_config.architectures[0] in ("DeepseekV2ForCausalLM", "DeepseekV3ForCausalLM"):
    ...
    if world_size > 1 and support_distributed_convert(hf_config):
        ...
        if any(arch in hf_config.architectures for arch in ("DeepseekV2ForCausalLM", "DeepseekV3ForCausalLM")):
            numel_partial: int = convert_checkpoint_from_transformers_to_megatron_deepseek(
    ...
    elif any(arch in hf_config.architectures for arch in ("DeepseekV2ForCausalLM", "DeepseekV3ForCausalLM")):
        convert_checkpoint_from_transformers_to_megatron_deepseek(
