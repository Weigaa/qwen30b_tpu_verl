...
usage() {
    cat <<'EOF'
Usage:
  ./run_mode1_local_shrinkaware_length_regroup.sh [extra hydra args...]

Shrink-aware local test:
  - uses elastic shrink mode=1 with floor=4
  - enables LengthAwareEpochSampler to regroup each batch by expected length
  - enables shrink-aware staged scheduling with shrink stages [8,4]
...
case "$floor" in
    4) ;;
    *)
        echo "shrink-aware local test is fixed to mode=1 shrink floor=4; got MODE1_FLOOR=$floor" >&2
        exit 2
        ;;
esac
...
export OUTPUT_SUBDIR="${OUTPUT_SUBDIR:-mode1_shrinkaware_length_regroup_floor4}"
...
DATASET_FRACTION="${DATASET_FRACTION:-0.003}"
EXTRA_OVERRIDES=()
EXTRA_OVERRIDES+=("data.dataset_fraction=$DATASET_FRACTION")

cd "$PATCH_TREE"
stamp=$(date -u +%Y%m%dT%H%M%SZ)
tee_log="$REPO_ROOT/mode1_local_shrinkaware_length_regroup_floor4_${stamp}.log"

printf '[mode1 local shrink-aware regroup] runtime_cwd=%s\n' "$PATCH_TREE"
printf '[mode1 local shrink-aware regroup] launcher=%s tee_log=%s\n' "$LAUNCHER" "$tee_log"
printf '[mode1 local shrink-aware regroup] model=%s distcp=%s train=%s test=%s\n' \
    "$MODEL_PATH" "$DISTCP_PATH" "$TRAIN_FILE" "$TEST_FILE"
printf '[mode1 local shrink-aware regroup] mode=1 floor=4 length_regroup=%s bucket=%s ema=%s stages=%s length_source=%s\n' \
    "$VLLM_EPOCH_LENGTH_REGROUP_ENABLE" \
    "$VLLM_EPOCH_LENGTH_REGROUP_BUCKET_SIZE" \
    "$VLLM_EPOCH_LENGTH_REGROUP_EMA_DECAY" \
    "$VLLM_ASCEND_SHRINK_AWARE_STAGES" \
    "$VLLM_ASCEND_SHRINK_AWARE_LENGTH_SOURCE"
printf '[mode1 local shrink-aware regroup] dataset_fraction=%s moe_stats=%s moe_stats_timing=%s native_topk_debug=%s\n' \
    "$DATASET_FRACTION" \
    "$VLLM_MOE_PATTERN_STATS" \
    "$VLLM_MOE_STATS_TIMING" \
    "$VLLM_ASCEND_NATIVE_MOE_TOPK_DEBUG"
printf '[mode1 local shrink-aware regroup] kv_cap=%s kv_cap_floor4=%s custom_kv_headroom=%s kv_init_headroom=%s custom_models=%s native_qwen3=%s\n' \
