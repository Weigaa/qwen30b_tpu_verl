...
_local_group_creation_lock = threading.Lock()
...
def _new_group_with_stable_local_name(
    ranks: list[int],
    backend: Union[str, Backend],
    local_synchronization_key: str,
) -> ProcessGroup:
    """Create a local-sync group without relying on process-local PG counts."""
    import torch.distributed.distributed_c10d as c10d

    expected_ranks = tuple(sorted(map(int, ranks)))

    def _stable_hash(group_ranks: list[int]) -> str:
        actual_ranks = tuple(sorted(map(int, group_ranks)))
        if actual_ranks != expected_ranks:
            raise RuntimeError(
                "local process-group ranks changed during creation: "
                f"expected={expected_ranks} actual={actual_ranks}")
        identity = (
            f"vllm-local-pg:{local_synchronization_key}:"
            f"{str(backend)}:{','.join(map(str, actual_ranks))}")
        return hashlib.sha1(identity.encode("utf-8"),
                            usedforsecurity=False).hexdigest()

    # PyTorch normally hashes ranks together with the process-local number of
    # live groups. That count can differ across independently shrinking EP
    # instances. Use an explicit transition identity while preserving the
    # public new_group implementation and backend setup.
    with _local_group_creation_lock:
        with patch.object(c10d, "_hash_ranks_to_str", _stable_hash):
            return torch.distributed.new_group(
                ranks,
                backend=backend,
                use_local_synchronization=True,
                group_desc=local_synchronization_key,
            )
...
class GroupCoordinator:
    ...
    def __init__(
        ...
        local_synchronization_key: Optional[str] = None,
    ):
        ...
        if not reuse_world_group:
            for ranks in group_ranks:
                if use_local_synchronization and local_synchronization_key:
                    device_group = _new_group_with_stable_local_name(
                        ranks,
                        torch_distributed_backend,
                        f"{local_synchronization_key}:device",
                    )
                else:
                ...
                if use_local_synchronization and local_synchronization_key:
                    cpu_group = _new_group_with_stable_local_name(
                        ranks,
                        "gloo",
                        f"{local_synchronization_key}:cpu",
                    )
                else:
...
def init_model_parallel_group(
    ...
    return GroupCoordinator(
        ...
        use_local_synchronization=use_local_synchronization,
        local_synchronization_key=local_synchronization_key,
    )
