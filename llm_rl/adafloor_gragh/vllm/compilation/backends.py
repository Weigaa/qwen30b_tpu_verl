...
class VllmBackend:
    ...
    def __call__(self, graph: fx.GraphModule, example_inputs) -> Callable:
        ...
        if not self.compilation_config.cache_dir:
            ...
            for filepath in forward_code_files:
                ...
                try:
                    ...
                    # Python 3.11 may report frozen stdlib modules (for
                    # example ``<frozen os>``) as traced source files. Keep
                    # their stable identifier in the hash, but there is no
                    # filesystem content to read.
        ...
        self._elastic_aclgraph_copy_call_count = 0

        if os.getenv("VLLM_ASCEND_ELASTIC_ACLGRAPH_DEBUG_INPUTS", "0") == "1":
            logger.warning(
                "Elastic ACLGraph outer input buffers initialized: "
                "rank=%s indices=%s shapes=%s dtypes=%s",
                rank,
                self.sym_tensor_indices,
                [tuple(example_inputs[x].shape)
                 for x in self.sym_tensor_indices],
                [str(example_inputs[x].dtype)
                 for x in self.sym_tensor_indices],
            )

        # this is the callable we return to Dynamo to run
        def copy_and_call(*args):
            self._elastic_aclgraph_copy_call_count += 1
            list_args = list(args)
            debug_values = []
            for i, index in enumerate(self.sym_tensor_indices):
                runtime_tensor = list_args[index]
                runtime_shape = runtime_tensor.shape[0]
                if runtime_shape > self.input_buffers[i].shape[0]:
                    old_shape = tuple(self.input_buffers[i].shape)
                    self.input_buffers[i] = runtime_tensor.new_empty(
                        (runtime_shape, *self.input_buffers[i].shape[1:]))
                    logger.info(
                        "Elastic ACLGraph outer input buffer expanded: "
                        "rank=%s index=%s old_shape=%s new_shape=%s",
                        rank,
                        index,
                        old_shape,
                        tuple(self.input_buffers[i].shape),
                    )
                ...
                if (os.getenv("VLLM_ASCEND_ELASTIC_ACLGRAPH_DEBUG_INPUTS", "0")
                        == "1" and rank == 0
                        and self._elastic_aclgraph_copy_call_count <= 20):
                    torch.npu.synchronize()
                    runtime_head = runtime_tensor.reshape(-1)[:4].cpu().tolist()
                    static_head = static_tensor.reshape(-1)[:4].cpu().tolist()
                    debug_values.append((index, runtime_head, static_head,
                                         runtime_tensor.data_ptr(),
                                         static_tensor.data_ptr()))

                # replace the tensor in the list_args to the static buffer
                list_args[index] = static_tensor
            if debug_values:
                logger.warning(
                    "Elastic ACLGraph outer input copy: rank=%s call=%s "
                    "values=%s",
                    rank,
                    self._elastic_aclgraph_copy_call_count,
                    debug_values,
                )
