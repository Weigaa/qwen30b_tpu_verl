...
def _support_torch_compile(
    ...
    def __call__(self, *args, **kwargs):
        ...
        # FULL_DECODE_ONLY deliberately has two execution paths. Uniform
        # single-token decode batches use the compiled static graph. Prefill
        # and mixed batches retain their runtime token shapes and must execute
        # the original eager forward. Otherwise the no-guard bytecode
        # dispatcher copies a prefill tensor into a decode-sized static input.
        if (self.vllm_config.compilation_config.cudagraph_mode
                == CUDAGraphMode.FULL_DECODE_ONLY):
            from vllm.forward_context import get_forward_context
            if (get_forward_context().cudagraph_runtime_mode
                    == CUDAGraphMode.NONE):
                return self.forward(*args, **kwargs)
