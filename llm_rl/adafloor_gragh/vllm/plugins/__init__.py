...
_SUPPRESSED_ASCEND_PLUGIN_PREFIX = "ascend_"
    ...
    for plugin in discovered_plugins:
        if allowed_plugins is None or plugin.name in allowed_plugins:
            ...
            try:
                ...
                if (plugin.name.startswith(_SUPPRESSED_ASCEND_PLUGIN_PREFIX)
                        and os.getenv("VLLM_SUPPRESS_ASCEND_PLUGIN_LOAD_ERRORS",
                                      "1").strip().lower()
                        not in ("0", "false", "no", "off")):
                    logger.debug("Suppressed failed optional Ascend plugin %s",
                                 plugin.name)
                    continue
