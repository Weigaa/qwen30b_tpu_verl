...
def _mode1_engine_kv_init_diag_enabled() -> bool:
    return os.getenv("VLLM_ASCEND_MODE1_ENGINE_KV_INIT_DIAG",
                     "0").strip().lower() in ("1", "true", "yes", "on")


def _mode1_engine_npu_memory_snapshot() -> dict[str, int | str]:
    if not hasattr(torch, "npu") or not torch.npu.is_available():
        return {"npu": "unavailable"}
    try:
        free_bytes, total_bytes = torch.npu.mem_get_info()
    except Exception:
        free_bytes, total_bytes = -1, -1
    snapshot: dict[str, int | str] = {
        "free": int(free_bytes),
        "total": int(total_bytes),
    }
    try:
        snapshot["torch_current"] = int(torch.npu.memory_allocated())
        snapshot["torch_reserved"] = int(torch.npu.memory_reserved())
    except Exception:
        pass
    if total_bytes >= 0 and free_bytes >= 0:
        snapshot["non_torch"] = int(
            max(total_bytes - free_bytes -
                int(snapshot.get("torch_current", 0)), 0))
    return snapshot

_R = TypeVar('_R')  # Return type for collective_rpc
print("### USING MY PATCHED vLLM core.py ###", flush=True)
...
class EngineCore:
    ...
    def _initialize_kv_caches(
        ...
        if _mode1_engine_kv_init_diag_enabled():
            logger.warning(
                "Mode1 engine KV init diag: stage=after_profile_before_config "
                "mode=%s floor=%s current_floor=%s current_kv=%s "
                "available_gpu_memory=%s kv_specs=%s memory=%s",
                os.getenv("VLLM_ASCEND_ELASTIC_EXECUTION_MODE"),
                os.getenv("VLLM_ASCEND_ELASTIC_MIN_COMPUTE_GROUP_SIZE"),
                os.getenv("VLLM_ASCEND_MODE1_PARITY_CURRENT_FLOOR"),
                os.getenv("VLLM_ASCEND_MODE1_PARITY_CURRENT_KV_TOKENS"),
                available_gpu_memory,
                len(kv_cache_specs),
                _mode1_engine_npu_memory_snapshot(),
            )
        ...
        if _mode1_engine_kv_init_diag_enabled():
            token_capacity = 0
            try:
                for config in self.kv_cache_configs:
                    ...
                    token_capacity = max(
                        token_capacity,
                        int(config.num_blocks) * int(min_block_size))
            except Exception:
                token_capacity = -1
            logger.warning(
                "Mode1 engine KV init diag: stage=before_initialize_from_config "
                "num_gpu_blocks=%s token_capacity_est=%s memory=%s",
                num_gpu_blocks,
                token_capacity,
                _mode1_engine_npu_memory_snapshot(),
            )
        try:
            ...
            if _mode1_engine_kv_init_diag_enabled():
                logger.exception(
                    "Mode1 engine KV init diag: initialize_from_config failed "
                    "memory=%s",
                    _mode1_engine_npu_memory_snapshot(),
                )
            raise
        if _mode1_engine_kv_init_diag_enabled():
            logger.warning(
                "Mode1 engine KV init diag: stage=after_initialize_from_config "
                "memory=%s",
                _mode1_engine_npu_memory_snapshot(),
            )
    ...
    def _mode1_effective_blocks_for_token_cap(
            self, kv_cache_config: KVCacheConfig,
            effective_target_tokens: int) -> tuple[int, int]:
        """Return the scheduler blocks/tokens produced by a mode=1 token cap."""
        if effective_target_tokens <= 0 or not kv_cache_config.kv_cache_groups:
            return 0, 0
        min_block_size = min(
            group.kv_cache_spec.block_size
            for group in kv_cache_config.kv_cache_groups)
        dcp_world_size = max(
            int(self.vllm_config.parallel_config.
                decode_context_parallel_size), 1)
        tokens_per_block_group = min_block_size * dcp_world_size
        if tokens_per_block_group <= 0:
            return kv_cache_config.num_blocks, self._kv_cache_token_capacity(
                kv_cache_config)
        num_groups = len(kv_cache_config.kv_cache_groups)
        blocks_per_group = effective_target_tokens // tokens_per_block_group
        target_blocks = blocks_per_group * num_groups
        if target_blocks <= 0:
            return 0, 0
        target_tokens = blocks_per_group * tokens_per_block_group
        return target_blocks, target_tokens
    ...
    def prepare_kv_cache_resume_for_mode1_step(
            self, target_floor: int | None = None) -> bool:
        """Run mode=1 pre-KV cleanup before rebuilding a released KV cache.

        Adaptive resize already performs this cleanup before allocating a new
        per-step KV cache.  A same-floor next step can still rebuild KV from an
        empty state after trainer-mode release, so it needs the same cleanup
        even when the floor and token cap did not change.
        """
        if not self._env_flag("VLLM_ASCEND_MODE1_PRE_RESUME_KV_CLEANUP", "1"):
            return False
        ...
        if target_floor is None:
            raw_floor = os.environ.get("VLLM_ASCEND_MODE1_PARITY_CURRENT_FLOOR")
        else:
            raw_floor = str(target_floor)
        if raw_floor:
            try:
                target_floor_int = int(float(raw_floor.strip()))
            except (TypeError, ValueError):
                logger.warning(
                    "Mode1 pre-resume KV cleanup ignores invalid target floor: %r",
                    raw_floor,
                )
                target_floor_int = None

        previous_floor_int: int | None = None
        previous_floor_env = (
            os.environ.get("VLLM_ASCEND_MODE1_PARITY_LAST_RESIZED_FLOOR")
            or os.environ.get("VLLM_ASCEND_MODE1_PARITY_CURRENT_FLOOR"))
        if previous_floor_env:
            try:
                previous_floor_int = int(float(previous_floor_env.strip()))
            except (TypeError, ValueError):
        ...
        def _log_comm_cache_state(tag: str) -> None:
            ...
            try:
                ...
                logger.exception(
                    "Mode1 pre-resume KV cleanup comm-cache state logging failed: "
                    "tag=%s",
                    tag,
                )
        ...
        start = time.time()
        logger.info(
            "Mode1 pre-resume KV cleanup phase=start target_floor=%s "
            "previous_floor=%s target_policy=%s",
            target_floor_int,
            previous_floor_int,
            target_policy,
        )
        _log_comm_cache_state("pre_resume_kv_cleanup_start")

        if target_floor_int is not None:
            logger.info(
                "Mode1 pre-resume KV cleanup phase=floor_prepare_start "
                "target_floor=%s previous_floor=%s",
                target_floor_int,
                previous_floor_int,
            )
            self.collective_rpc("prepare_mode1_step_floor_for_kv_resize",
                                args=(target_floor_int, previous_floor_int))
            gc.collect()
            logger.info(
                "Mode1 pre-resume KV cleanup phase=floor_prepare_done "
                "target_floor=%s",
                target_floor_int,
            )
            _log_comm_cache_state("pre_resume_after_floor_prepare")
            if (target_policy not in ("planned", "fixed", "plan")
                    and self._env_flag(
                        "VLLM_ASCEND_MODE1_NATURAL_PRUNE_RUNTIME_ON_KV_RESIZE",
                        "1")):
                logger.info(
                    "Mode1 pre-resume KV cleanup "
                    "phase=natural_runtime_prune_start target_floor=%s",
                    target_floor_int,
                )
                self.collective_rpc(
                ...
                logger.info(
                    "Mode1 pre-resume KV cleanup "
                    "phase=natural_runtime_prune_done target_floor=%s",
                    target_floor_int,
                )
                _log_comm_cache_state("pre_resume_after_natural_runtime_prune")

        self.collective_rpc("mode1_resize_world_barrier",
                            args=("pre_resume_after_floor_prepare", ))

        logger.info("Mode1 pre-resume KV cleanup phase=clear_kv_refs_start")
        self.collective_rpc("clear_kv_cache_for_resize")
        gc.collect()
        logger.info("Mode1 pre-resume KV cleanup phase=clear_kv_refs_done")
        _log_comm_cache_state("pre_resume_after_clear_kv_refs")
        self.collective_rpc("mode1_resize_world_barrier",
                            args=("pre_resume_after_clear_kv_refs", ))

        if self._env_flag(
                "VLLM_ASCEND_MODE1_CLEAR_STALE_PARAM_DICTS_BEFORE_KV_RESUME",
                "1"):
            logger.info(
                "Mode1 pre-resume KV cleanup "
                "phase=clear_stale_param_dicts_start")
            self.collective_rpc(
                "clear_mode1_stale_fullname_parameter_dicts_for_kv_resize")
            gc.collect()
            logger.info(
                "Mode1 pre-resume KV cleanup "
                "phase=clear_stale_param_dicts_done")
            self.collective_rpc("mode1_resize_world_barrier",
                                args=("pre_resume_after_clear_stale_param_dicts", ))
            _log_comm_cache_state("pre_resume_after_clear_stale_param_dicts")
        else:
            logger.info(
                "Mode1 pre-resume KV cleanup "
                "phase=clear_stale_param_dicts_skipped reason=disabled")
        ...
        try:
            ...
            logger.info(
                "Mode1 pre-resume KV cleanup available memory: "
                "current_available_values=%s headroom_bytes=%s",
                available_values,
                resize_headroom,
            )
        except Exception:
            logger.exception(
                "Mode1 pre-resume KV cleanup failed to query available memory")

        logger.info(
            "Mode1 pre-resume KV cleanup phase=done target_floor=%s "
            "previous_floor=%s elapsed_s=%.2f",
            target_floor_int,
            previous_floor_int,
            time.time() - start,
        )
    ...
    def resize_kv_cache_for_mode1_step(self,
        ...
        target_blocks, rounded_target_tokens = (
            self._mode1_effective_blocks_for_token_cap(
                old_scheduler_config, effective_target_tokens))
        dp_world_size = max(
        ...
        current_floor_env = os.environ.get(
            "VLLM_ASCEND_MODE1_PARITY_CURRENT_FLOOR")
        last_resized_floor_env = os.environ.get(
            "VLLM_ASCEND_MODE1_PARITY_LAST_RESIZED_FLOOR")
        previous_floor_env = last_resized_floor_env or current_floor_env
        previous_floor_int: int | None = None
        if previous_floor_env is not None:
            try:
                previous_floor_int = int(previous_floor_env.strip())
            except (TypeError, ValueError):
                previous_floor_int = None
        upward_floor_transition = (
            previous_floor_int is not None and target_floor_int is not None
            and target_floor_int > previous_floor_int
            and previous_floor_int > 0)
        same_floor_transition = (
            previous_floor_int is not None and target_floor_int is not None
            and target_floor_int == previous_floor_int
            and previous_floor_int > 0)
        current_floor_matches = (
            target_floor_int is None or
            (current_floor_env is not None
             and current_floor_env.strip() == str(target_floor_int)))
        force_resize = self._env_flag(
            "VLLM_ASCEND_MODE1_FORCE_KV_RESIZE_ON_NEXT_GENERATE", "0")
        if force_resize:
            logger.info(
                "Proceed mode1 adaptive KV resize because force flag is set: "
                "old_tokens=%s target_tokens=%s effective_target_tokens=%s "
                "target_floor=%s previous_floor=%s current_floor=%s",
                old_tokens,
                target_tokens,
        ...
        if (same_floor_transition and old_tokens == effective_target_tokens
                and not planned_expert_prefill_on_resize
                and not force_resize):
            os.environ["VLLM_ASCEND_MODE1_PARITY_CURRENT_KV_TOKENS"] = str(
                old_tokens)
            os.environ["VLLM_ASCEND_MODE1_PARITY_CURRENT_FLOOR"] = str(
                target_floor_int)
            os.environ["VLLM_ASCEND_MODE1_PARITY_LAST_RESIZED_FLOOR"] = str(
                target_floor_int)
            logger.info(
                "Skip mode1 adaptive KV resize because floor and KV target are "
                "unchanged: old_tokens=%s target_tokens=%s "
                "effective_target_tokens=%s target_floor=%s previous_floor=%s "
                "current_floor=%s old_blocks=%s target_blocks=%s",
                old_tokens,
                target_tokens,
                effective_target_tokens,
                target_floor_int,
            ...
            _log_comm_cache_state("resize_skip_same_floor")
            return False
        if (old_tokens == effective_target_tokens and current_floor_matches
                and not planned_expert_prefill_on_resize):
            if force_resize:
                logger.info(
                    "Bypass mode1 adaptive KV resize same-target skip because "
                    "force flag is set: old_tokens=%s target_tokens=%s "
                    "target_floor=%s",
                    old_tokens,
                    target_tokens,
                    target_floor_int,
                ...
                return False
        if (old_blocks == target_blocks and current_floor_matches
                and not planned_expert_prefill_on_resize
                and self._env_flag(
                    "VLLM_ASCEND_MODE1_SKIP_SAME_BLOCK_KV_RESIZE", "1")):
            if force_resize:
                logger.info(
                    "Bypass mode1 adaptive KV resize same-block skip because "
                    "force flag is set: old_blocks=%s target_blocks=%s "
                    "target_floor=%s",
                    old_blocks,
                    target_blocks,
                    target_floor_int,
                ...
                os.environ["VLLM_ASCEND_MODE1_PARITY_CURRENT_KV_TOKENS"] = str(
                    rounded_target_tokens or old_tokens)
                if target_floor_int is not None:
                    os.environ["VLLM_ASCEND_MODE1_PARITY_CURRENT_FLOOR"] = str(
                        target_floor_int)
                    os.environ["VLLM_ASCEND_MODE1_PARITY_LAST_RESIZED_FLOOR"] = str(
                        target_floor_int)
                logger.info(
                    "Skip mode1 adaptive KV resize because target maps to the "
                    "current KV block count: old_tokens=%s target_tokens=%s "
                    "effective_target_tokens=%s rounded_target_tokens=%s "
                    "target_floor=%s current_floor=%s old_blocks=%s "
                    "target_blocks=%s",
                    old_tokens,
                    target_tokens,
                    effective_target_tokens,
                    rounded_target_tokens,
                    target_floor_int,
                    current_floor_env,
                    old_blocks,
                    target_blocks,
                )
                _log_comm_cache_state("resize_skip_same_blocks")
                return False
        ...
        start = time.time()
        os.environ["VLLM_ASCEND_MODE1_PARITY_CURRENT_KV_TOKENS"] = str(
            rounded_target_tokens or effective_target_tokens)
        if target_floor_int is not None:
            os.environ["VLLM_ASCEND_MODE1_PARITY_CURRENT_FLOOR"] = str(
                target_floor_int)
        logger.info(
            ...
            "predicted_target_blocks=%s rounded_target_tokens=%s",
            old_tokens,
            target_tokens,
            effective_target_tokens,
            target_floor_int,
        ...
        old_kv_cleared_before_floor_prepare = False
        if (upward_floor_transition and self._env_flag(
                "VLLM_ASCEND_MODE1_CLEAR_OLD_KV_BEFORE_UPWARD_FLOOR_PREPARE",
                "1")):
            # Upward transitions such as floor2->floor4 compact temporary MoE
            # storage before allocating the new KV cache. Release the previous
            # step's KV first so compact does not peak on top of old KV.
            logger.info(
                "Mode1 adaptive KV resize "
                "phase=clear_old_kv_before_floor_prepare_start "
                "target_floor=%s previous_floor=%s old_tokens=%s",
                target_floor_int,
                previous_floor_int,
                old_tokens,
            )
            self.collective_rpc("clear_kv_cache_for_resize")
            gc.collect()
            old_kv_cleared_before_floor_prepare = True
            logger.info(
                "Mode1 adaptive KV resize "
                "phase=clear_old_kv_before_floor_prepare_done "
                "target_floor=%s previous_floor=%s old_tokens=%s",
                target_floor_int,
                previous_floor_int,
                old_tokens,
            )
            _log_comm_cache_state("after_clear_old_kv_before_floor_prepare")
            self.collective_rpc(
                "mode1_resize_world_barrier",
                args=("after_clear_old_kv_before_floor_prepare", ))
            if self._env_flag("VLLM_ASCEND_MODE1_KV_RESIZE_LIVE_TENSOR_SCAN",
                              "0"):
                self.collective_rpc(
                    "mode1_resize_live_tensor_scan",
                    args=("after_clear_old_kv_before_floor_prepare", ))
        if target_floor_int is not None:
            logger.info(
        ...
        if old_kv_cleared_before_floor_prepare:
            logger.info(
                "Mode1 adaptive KV resize phase=clear_old_kv_skipped "
                "reason=already_cleared_before_floor_prepare")
        else:
        ...
        logger.info(
            ...
            "target_tokens=%s effective_target_tokens=%s new_tokens=%s "
            "old_blocks=%s new_blocks=%s previous_floor=%s "
            "target_floor=%s elapsed_s=%.2f",
            old_tokens,
            target_tokens,
            effective_target_tokens,
            new_tokens,
        ...
        if target_floor_int is not None:
            os.environ["VLLM_ASCEND_MODE1_PARITY_LAST_RESIZED_FLOOR"] = str(
                target_floor_int)
        if force_resize:
            os.environ.pop("VLLM_ASCEND_MODE1_FORCE_KV_RESIZE_ON_NEXT_GENERATE",
                           None)
        return True
...
class DPEngineCoreProc(EngineCoreProc):
    ...
    def run_busy_loop(self):
        ...
        # Loop until process is sent a SIGINT or SIGTERM
        # print("not define", abc)
        begin_rollout_time = time.time()
        print("rank", self.dp_rank, "begin rollouts time", begin_rollout_time)
        rollouts_flag = True
        while True:
            # 1) Poll the input queue until there is work to do.
            self._process_input_queue()
            ...
            if not executed:
                if rollouts_flag:
                    ...
                    print("rank", self.dp_rank, "real rollouts time is", end_real_time - begin_rollout_time)
                    rollouts_flag = False
                if not local_unfinished_reqs and not self.engines_running:
                    end_dummy_time = time.time()
                    print("rank", self.dp_rank, "dummy rollouts time is", end_dummy_time - end_real_time)
...
class DPEngineCoreActor(DPEngineCoreProc):
    ...
    def run(self):
        ...
        """
        try:
            print("### ENTER DPEngineCoreActor.run ###", flush=True)
            self.run_busy_loop()
        except SystemExit:
            logger.debug("EngineCore exiting.")
            raise
