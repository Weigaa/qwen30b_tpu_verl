...
class LLMEngine:
    ...
    def __init__(
        ...
        if self.external_launcher_dp:
            ...
            gathered_ranks = [
                torch.empty_like(rank_tensor) for _ in range(dp_size)
            ]
            torch.distributed.all_gather(
                gathered_ranks, rank_tensor, group=dp_pg)
        ...
        # Elastic no-dummy execution is required for ranks to leave the active
        # DP group. PIECEWISE elastic ACLGraph is safe here because attention
        # and elastic MoE remain graph boundaries and consume the live process
        # groups. Other compiled modes still fail closed on the eager check.
        # If the configured floor already equals the initial DP size (for
        # example 16 -> 16), retain the original dummy-batch behavior.
        elastic_decode_mode = (
            self.model_config.enforce_eager
            or envs_ascend.VLLM_ASCEND_ELASTIC_ACLGRAPH
        )
        self.elastic_ep_no_dummy = (
            envs_ascend.VLLM_ASCEND_ENABLE_ELASTIC_PARALLEL_SHRINK
            and self.external_launcher_dp and elastic_decode_mode
    ...
    def _original_dp_global_ranks(self, dp_world_size: int) -> list[int]:
        ranks = getattr(self, "elastic_original_dp_global_ranks", None)
        if ranks:
            return [int(rank) for rank in ranks]
        return list(range(dp_world_size))

    def _dp_global_to_local_ranks(
            self, global_ranks: list[int], dp_world_size: int) -> list[int]:
        original = self._original_dp_global_ranks(dp_world_size)
        local_by_global = {
        ...
        if unknown:
            raise ValueError(
                "active ranks fall outside the original external-DP group: "
                f"active={global_ranks} original={original} unknown={unknown}")
        return sorted(local_by_global[int(rank)] for rank in global_ranks)

    def _dp_local_to_global_ranks(
            self, local_ranks: list[int], dp_world_size: int) -> list[int]:
        original = self._original_dp_global_ranks(dp_world_size)
        if any(int(rank) < 0 or int(rank) >= len(original)
               for rank in local_ranks):
            raise ValueError(
                f"local ranks {local_ranks} are invalid for external-DP "
                f"group {original}")
    ...
    def _staged_shrink_target(self, natural_active_ranks: list[int],
        ...
        current_global = (list(self.elastic_ep_active_ranks)
                          if self.elastic_ep_active_ranks is not None
                          else full_world_ranks)
        current = self._dp_global_to_local_ranks(
            current_global, full_world_size)
        unfinished = self._dp_global_to_local_ranks(
            natural_active_ranks, full_world_size)
        decision = decide_staged_shrink(
            enabled=envs_ascend.VLLM_ASCEND_SHRINK_AWARE_ENABLE,
            mode=envs_ascend.VLLM_ASCEND_SHRINK_AWARE_MODE,
            current_active_ranks=current,
            unfinished_ranks=unfinished,
            role_plan=plan,
        ...
        if decision.should_shrink:
            target_global_ranks = self._dp_local_to_global_ranks(
                decision.target_active_ranks, full_world_size)
            logger.info(
                "Shrink-aware staged trigger: stage=%s current_local=%s "
                "unfinished_local=%s target_local=%s target_global=%s",
                decision.stage_name, current, unfinished,
                decision.target_active_ranks, target_global_ranks)
            return True, target_global_ranks
        if not has_unfinished and natural_active_ranks:
            self.should_execute_dummy_batch = True
        return True, list(current_global)
    ...
    def _should_enter_single_rank_no_ep_tail(self, num_active_ranks: int,
        ...
        if num_active_ranks != 1 or dp_world_size != 2:
            return False
        if envs_ascend.VLLM_ASCEND_ELASTIC_EXECUTION_MODE not in (2, 3, 4, 5):
            return False
    ...
    def _get_runtime_elastic_min_compute_group_size(self) -> Optional[int]:
        ...
        mode = envs_ascend.VLLM_ASCEND_ELASTIC_EXECUTION_MODE
        if mode not in (4, 5):
        ...
        if not isinstance(original_dp_world_size, int) or original_dp_world_size <= 0:
            raise ValueError(
                f"{env_name} requires "
                "external-launcher DP to be initialized.")
        if runtime_floor > original_dp_world_size:
            raise ValueError(
                f"{env_name} cannot exceed "
                f"the initial DP size: {runtime_floor} > {original_dp_world_size}.")
        if original_dp_world_size % runtime_floor != 0:
            raise ValueError(
                f"{env_name} must divide "
                f"the initial DP size: {runtime_floor} vs {original_dp_world_size}.")
