...
def check_enough_kv_cache_memory(vllm_config: VllmConfig,
    ...
    if (os.getenv("VLLM_ASCEND_ELASTIC_EXECUTION_MODE", "0") == "1"
            and _env_flag("VLLM_ASCEND_MODE1_IN_COLD_ENGINE_INIT", "0")
            and _env_flag("VLLM_ASCEND_MODE1_USE_COLD_INIT_KV_CAP", "1")):
        logger.warning(
            "Skipping profile-byte KV memory precheck for mode1 cold engine "
            "init: available_memory=%s max_model_len=%s cold_init_tokens=%s. "
            "The cold KV cache is only used to construct the vLLM engine and "
            "will be rebuilt by rollout resume with the real step budget.",
            available_memory,
            vllm_config.model_config.max_model_len,
            os.getenv("VLLM_ASCEND_MODE1_COLD_INIT_KV_TOKENS", ""),
        )
        return

    if (os.getenv("VLLM_ASCEND_ELASTIC_EXECUTION_MODE", "0") == "1"
            and _env_flag("VLLM_ASCEND_MODE1_USE_EXPLICIT_KV_CAP_FOR_INIT",
                          "0")):
        logger.warning(
            "Skipping profile-byte KV memory precheck for mode1 explicit KV "
            "cap init: available_memory=%s max_model_len=%s. The final KV "
            "block count will be bounded by the configured floor token cap.",
            available_memory,
            vllm_config.model_config.max_model_len,
        )
        return
...
def maybe_cap_mode1_parity_num_blocks(
    ...
    cold_init_env = os.getenv("VLLM_ASCEND_MODE1_COLD_INIT_KV_TOKENS")
    in_cold_engine_init = (
        _env_flag("VLLM_ASCEND_MODE1_IN_COLD_ENGINE_INIT", "0")
        and _env_flag("VLLM_ASCEND_MODE1_USE_COLD_INIT_KV_CAP", "1")
        and cold_init_env is not None and cold_init_env.strip())
    cap_source = f"floor{configured_floor}"
    if in_cold_engine_init:
        try:
            max_tokens = int(float(cold_init_env))
            cap_source = "cold_engine_init"
        except ValueError:
            logger.warning(
                "Ignoring invalid mode1 cold init KV token cap: %r",
                cold_init_env)
            max_tokens = None
    elif adaptive_env is not None and adaptive_env.strip():
        try:
    ...
    if num_blocks <= 0:
        allow_zero_profile_recovery = (
            in_cold_engine_init
            or _env_flag("VLLM_ASCEND_MODE1_USE_EXPLICIT_KV_CAP_FOR_INIT",
                         "0"))
        if not allow_zero_profile_recovery:
            return num_blocks
        capped_tokens = (max_blocks // num_groups * min_block_size *
                         max(int(dcp_world_size), 1))
        logger.warning(
            "Using explicit mode1 KV cap to recover from zero profiled KV "
            "blocks: profiled_blocks=%s recovered_blocks=%s block_size=%s "
            "groups=%s dcp_world_size=%s recovered_tokens=%s max_tokens=%s "
            "configured_floor=%s effective_floor=%s cap_source=%s "
