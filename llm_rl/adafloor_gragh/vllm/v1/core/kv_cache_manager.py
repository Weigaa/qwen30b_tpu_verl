...
class KVCacheManager:
    ...
    def allocate_slots(
        ...
        if num_blocks_to_allocate > free_blocks:
            if os.getenv("VLLM_ASCEND_MODE1_KV_PREEMPT_DIAG", "0") == "1":
                current_blocks = self.coordinator.get_blocks(request.request_id)
                current_block_count = sum(len(group)
                                          for group in current_blocks)
                total_blocks = getattr(self.block_pool, "num_gpu_blocks",
                                       "unknown")
                logger.warning(
                    "[mode1_kv_diag] allocate_slots_failed request_id=%s "
                    "needed_blocks=%s free_blocks=%s total_blocks=%s "
                    "current_blocks=%s num_new_tokens=%s "
                    "num_tokens_need_slot=%s num_computed_tokens=%s "
                    "num_new_computed_tokens=%s num_lookahead_tokens=%s "
                    "max_model_len=%s",
                    request.request_id,
