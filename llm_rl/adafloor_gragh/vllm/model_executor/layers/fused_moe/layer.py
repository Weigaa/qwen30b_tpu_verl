...
class FusedMoE(CustomOp):
    ...
    def _load_w13(self,
        ...
        shard_size = expert_data.shape[target_shard_dim] // 2
        if not load_full:
        ...
        expert_row = expert_data
        if shard_id == "w1":
            expert_data = expert_data.narrow(target_shard_dim, 0, shard_size)
        # w3, up_proj: Load into second logical weight of w13.
        else:
            assert shard_id == "w3"
            expert_data = expert_data.narrow(target_shard_dim, shard_size,
                                             shard_size)
        expert_data = self._prepare_expert_weight_write_target(
            expert_row, expert_data)
    ...
    def _load_w2(self,
        ...
        preserve_expert_dim = (
            ...
            and int(expert_data.shape[0]) == 1)
        target_shard_dim = shard_dim + (1 if preserve_expert_dim else 0)
        shard_size = expert_data.shape[target_shard_dim]
        if not load_full:
            loaded_weight = loaded_weight.narrow(shard_dim,
                                                 shard_size * tp_rank,
                                                 shard_size)
    ...
    @staticmethod
    def _mode1_parse_int_filter(value: str) -> Optional[set[int]]:
        value = value.strip()
    ...
    @staticmethod
    def _mode1_tensor_fingerprint(tensor: Optional[torch.Tensor],
                                  max_values: int = 4) -> dict:
        if tensor is None:
        ...
        try:
            ...
            view = detached.reshape(-1)
            sample_count = max(1, min(int(max_values), numel))
            if sample_count == 1:
                offsets = [0]
            else:
                offsets = [
                    int(round(i * (numel - 1) / (sample_count - 1)))
                    for i in range(sample_count)
                ]
            head = [round(float(view[idx].float().item()), 6)
                    for idx in offsets]
            abs_view = view.float().abs()
            abs_max = float(abs_view.max().item())
            return {
                "ok": True,
                "shape": tuple(detached.shape),
                ...
                "numel": numel,
                "sum": round(float(view.float().sum().item()), 6),
                "abs_mean": round(float(abs_view.mean().item()), 6),
                "abs_max": round(abs_max, 6),
                "zero_like": abs_max == 0.0,
    ...
    def _mode1_weight_loader_diag_enabled(self, original_expert_id: int,
                                          local_expert_id: int,
                                          weight_name: str,
                                          shard_id: str) -> bool:
        if os.environ.get("VLLM_ASCEND_MODE1_WEIGHT_LOADER_DIAG",
                          "0").lower() not in ("1", "true", "yes", "on"):
            return False
        layer_filter = self._mode1_parse_int_filter(os.environ.get(
            "VLLM_ASCEND_MODE1_WEIGHT_LOADER_DIAG_LAYERS", "0"))
        if (layer_filter is not None
                and getattr(self, "layer_idx", -1) not in layer_filter):
            return False
        rank_filter = self._mode1_parse_int_filter(os.environ.get(
            "VLLM_ASCEND_MODE1_WEIGHT_LOADER_DIAG_EP_RANKS", "0"))
        if rank_filter is not None and int(getattr(self, "ep_rank",
                                                   -1)) not in rank_filter:
            return False
        expert_filter = self._mode1_parse_int_filter(os.environ.get(
            "VLLM_ASCEND_MODE1_WEIGHT_LOADER_DIAG_EXPERTS",
            "0,1,2,3,4,5,6,7"))
        if (expert_filter is not None and int(original_expert_id)
                not in expert_filter and int(local_expert_id)
                not in expert_filter):
            return False
        shard_filter = os.environ.get(
            "VLLM_ASCEND_MODE1_WEIGHT_LOADER_DIAG_SHARDS", "w1,w2,w3")
        if shard_filter.strip() != "*":
            allowed_shards = {
                item.strip()
                for item in shard_filter.split(",")
                if item.strip()
            }
            if shard_id not in allowed_shards:
                return False
        name_filter = os.environ.get(
            "VLLM_ASCEND_MODE1_WEIGHT_LOADER_DIAG_NAME_CONTAINS", "weight")
        return not name_filter or name_filter in weight_name

    def _mode1_effective_loaded_weight_for_diag(self, loaded_weight: torch.Tensor,
                                                expert_data: torch.Tensor,
                                                shard_dim: int,
                                                shard_id: str,
                                                load_full_w2: bool = False
        ...
        try:
            ...
            if shard_id == "w2":
                ...
                shard_size = int(expert_data.shape[target_shard_dim])
                return loaded_weight.narrow(shard_dim,
                                            shard_size * int(self.tp_rank),
                                            shard_size)
            if shard_id in ("w1", "w3"):
    ...
    def weight_loader(self,
        ...
        original_expert_id = int(expert_id)
        ...
        if preserve_expert_dim_write:
            expert_data = self._prepare_expert_weight_write_target(
                param.data, expert_data)
        weight_loader_diag = self._mode1_weight_loader_diag_enabled(
            original_expert_id=original_expert_id,
        ...
        # Case model weights
        if "weight" in weight_name:
            if weight_loader_diag:
                values = int(os.environ.get(
                    "VLLM_ASCEND_MODE1_WEIGHT_LOADER_DIAG_VALUES", "4"))
                effective_loaded = self._mode1_effective_loaded_weight_for_diag(
                    loaded_weight=loaded_weight,
                    expert_data=expert_data,
                    shard_dim=shard_dim,
                    shard_id=shard_id,
                    load_full_w2=getattr(param, "load_full_w2", False))
                logger.warning(
                    "Mode1 weight_loader diag before: layer=%s ep_rank=%s "
                    "tp_rank=%s original_expert=%s local_expert=%s "
                    "weight_name=%s shard=%s full_load=%s "
                    "preserve_expert_dim_write=%s param_shape=%s "
                    "expert_data_shape=%s loaded_received=%s "
                    "loaded_effective=%s target_before=%s",
                    getattr(self, "layer_idx", -1),
                    getattr(self, "ep_rank", None),
                    ...
                    self._mode1_tensor_fingerprint(loaded_weight, values),
                    self._mode1_tensor_fingerprint(effective_loaded, values),
                    self._mode1_tensor_fingerprint(expert_data, values),
                )
            self._load_model_weight_or_group_weight_scale(
                shard_id=shard_id,
                shard_dim=shard_dim,
            ...
            if weight_loader_diag:
                values = int(os.environ.get(
                    "VLLM_ASCEND_MODE1_WEIGHT_LOADER_DIAG_VALUES", "4"))
                logger.warning(
                    "Mode1 weight_loader diag after: layer=%s ep_rank=%s "
                    "tp_rank=%s original_expert=%s local_expert=%s "
                    "weight_name=%s shard=%s target_after=%s",
                    getattr(self, "layer_idx", -1),
                    getattr(self, "ep_rank", None),
                    getattr(self, "tp_rank", None),
                    original_expert_id,
