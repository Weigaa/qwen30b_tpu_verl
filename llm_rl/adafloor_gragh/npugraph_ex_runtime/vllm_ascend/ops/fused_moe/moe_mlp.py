...
def unified_apply_mlp(hidden_states: torch.Tensor,
                      ...
                      dynamic_eplb: bool = False) -> torch.Tensor:
    # CANN 8.5 requires the tensor group-list ABI of grouped matmul to use
    # int64. Some dispatch kernels return int32 counts, so normalize once at
    # the common MLP boundary for both eager execution and npugraph_ex.
    if group_list.dtype != torch.int64:
        group_list = torch_npu.npu_dtype_cast(group_list, torch.int64)
    if with_quant:
