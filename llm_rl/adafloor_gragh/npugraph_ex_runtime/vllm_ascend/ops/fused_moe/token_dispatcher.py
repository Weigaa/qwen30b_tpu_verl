...
class TokenDispatcherWithMC2(MoETokenDispatcher):
    ...
    def __init__(self, **kwargs):
        ...
        configured_global_bs = int(os.getenv("VLLM_ASCEND_MC2_GLOBAL_BS", "0"))
        if configured_global_bs >= 0:
            # Keep the old eager MC2 operator contract by default.  A3
            # dispatch_v2 accepts global_bs=0, while a fixed value must be
            # max_bs_per_rank * ep_world_size and can over-reserve workspace.
            self.global_bs = configured_global_bs
        else:
...
class TokenDispatcherWithAllGather(MoETokenDispatcher):
    ...
    def token_dispatch(self,
        ...
        # Materialize routing counts with the dtype required by grouped matmul.
        # A plain aten.to can be elided incorrectly by GraphEX for this output.
        expert_tokens = torch_npu.npu_dtype_cast(expert_tokens, torch.int64)
