...
from vllm.model_executor.layers.fused_moe.layer import (
    ...
    determine_expert_map as _vllm_determine_expert_map)
...
from vllm_ascend.utils import (get_all_reduce_merge_state,
                               get_rm_router_logits_state, is_310p,
                               maybe_trans_nz, vllm_version_is)
...
def determine_expert_map(ep_size: int,
                         ep_rank: int,
                         num_experts: int,
                         layer_idx: Optional[int] = None):
    del layer_idx
    local_num_experts, expert_map, _ = _vllm_determine_expert_map(
        ep_size=ep_size,
        ep_rank=ep_rank,
        global_num_experts=num_experts,
    )
    log2phy = determine_default_log2phy_map(num_experts, ep_size, ep_rank, 0)
    return local_num_experts, expert_map, log2phy
...
class AscendUnquantizedFusedMoEMethod(UnquantizedFusedMoEMethod):
    ...
    def _execute_single_wave(
        ...
        if (log2phy is not None
            ...
            # The remap is validated eagerly when the elastic layout is built.
            # A Tensor-valued Python branch cannot be represented by the FULL
            # npugraph_ex trace, so retain the defensive runtime check outside
            # compilation without baking it into the graph.
    ...
    def process_weights_after_loading(self, layer):
        ...
        if layer.w13_weight.device.type == "cpu":
            ...
            # Honor the same format switch as upstream vLLM-Ascend. GraphEX
            # requires the ND grouped-matmul path when NZ is explicitly off.
...
class AscendFusedMoE(FusedMoE):
    ...
    @property
    def expert_map(self) -> Optional[torch.Tensor]:
        return (getattr(self, "expert_mask", None)
                if getattr(self, "rocm_aiter_fmoe_enabled", False) else
                getattr(self, "_expert_map", None))

    @expert_map.setter
    def expert_map(self, value: Optional[torch.Tensor]) -> None:
        if getattr(self, "rocm_aiter_fmoe_enabled", False):
            self.expert_mask = value
            return

        # vLLM 0.14 exposes expert_map as a read-only property backed by
        # _expert_map. The legacy Ascend path mutates expert_map during EPLB /
        # elastic remapping, so keep those old call sites working by writing the
        # backing buffer directly.
        if "_buffers" not in self.__dict__:
            object.__setattr__(self, "_expert_map", value)
        elif "_expert_map" in self._buffers:
            self._buffers["_expert_map"] = value
        elif "_expert_map" in self.__dict__:
            object.__setattr__(self, "_expert_map", value)
        else:
            self.register_buffer("_expert_map", value)
    ...
    def __init__(
        ...
        routing_method_type: Optional[Any] = None,
        router_logits_dtype: Optional[torch.dtype] = None,
        ...
        self.moe_parallel_config = FusedMoEParallelConfig.make(
            ...
            pcp_size_=(pcp_size
                       if pcp_size is not None else get_pcp_group().world_size),
            dp_size_=(dp_size
    ...
    def restore_lossless_full_world_primary_layout(self) -> None:
        ...
        (self.active_local_num_experts, self.expert_map,
         self.log2phy) = determine_expert_map(
             self.ep_size,
             self.ep_rank,
             logical_num_experts,
    ...
    def export_lossless_expert_npu_weights(
        ...
        if len(local_slots) == 1:
            ...
            return (source_w13[local_slot:local_slot + 1],
                    source_w2[local_slot:local_slot + 1])
    ...
    def activate_lossless_local_experts(self, active_expert_ids: list[int],
        ...
        if getattr(self, "lossless_zero_redundancy_preallocated_loaded", False):
            ...
            for local_slot, source_local_id in enumerate(source_local_ids):
                if source_local_id >= 0:
                    if local_slot != source_local_id:
                        raise RuntimeError(
                            f"experts to stay in-place at layer={self.layer_idx}: "
                            f"slot={local_slot}, source_local_id={source_local_id}")
                    continue
                if cpu_expert_weights is None:
                    continue
    ...
    def forward(self,
        ...
        debug_enabled = bool(self.elastic_debug_budget > 0 and not is_dummy
                             and not getattr(forward_context, "in_profile_run",
                                             False))
        if debug_enabled:
            local_expert_ids = []
            if self.expert_map is not None:
...
def ascend_legacy_moe_forward(
    hidden_states: torch.Tensor,
    router_logits: torch.Tensor,
    layer_name: str,
) -> torch.Tensor:
    """Execute topology-aware legacy MoE behind an opaque graph boundary."""
    forward_context = get_forward_context()
...
def ascend_legacy_moe_forward_fake(
    hidden_states: torch.Tensor,
    router_logits: torch.Tensor,
    layer_name: str,
) -> torch.Tensor:
...
direct_register_custom_op(
    op_name="ascend_legacy_moe_forward",
    op_func=ascend_legacy_moe_forward,
    mutates_args=["hidden_states"],
    fake_impl=ascend_legacy_moe_forward_fake,
