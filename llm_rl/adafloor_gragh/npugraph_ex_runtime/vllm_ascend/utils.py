...
def sleep_mode_enabled():
    ...
    if _SLEEP_MODE_ENABLED is None:
        try:
            ...
            _SLEEP_MODE_ENABLED = bool(getattr(_build_info, "__sleep_mode_enabled__", True))
        except Exception:
...
def register_ascend_customop(vllm_config: VllmConfig | None = None):
    ...
    if envs_ascend.VLLM_ASCEND_USE_COMMON_FUSED_MOE:
        ...
        # registered for models that need that name.
...
# Backward-compatible aliases used by the legacy elastic MoE runtime modules.
AscendSocVersion = AscendDeviceType
...
def init_ascend_soc_version():
    return get_ascend_soc_version()


def get_rm_router_logits_state(ep_size: int, dp_size: int, is_deepseek_v3_r1: bool):
    fused_allgather_ep = bool(getattr(envs_ascend, "VLLM_ENABLE_FUSED_EXPERTS_ALLGATHER_EP", False))
    if dp_size > 1:
        if fused_allgather_ep and ep_size > 1 and is_deepseek_v3_r1:
            return True
        if ep_size == 1 and is_deepseek_v3_r1:
            return True
    return False
...
def refresh_block_size(vllm_config):
    ...
    legacy_attention = envs_ascend.VLLM_ASCEND_USE_LEGACY_ATTENTION
    if cache_config.block_size is None:
        cache_config.block_size = 64 if legacy_attention else 128
    ...
    if legacy_attention:
        if cache_config.block_size != 64:
            logger.info("Legacy Ascend attention uses KV block size 64.")
