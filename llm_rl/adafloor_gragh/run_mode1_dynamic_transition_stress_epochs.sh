...
Key environment variables:
  ...
  DYNAMIC_HISTORY_STEPS=5        Number of steps required from the reusable
                                 history. This may be smaller than the stress
                                 run plan without weakening output validation.
...
if [[ "$DYNAMIC_SHRINK_POLICY" == "planned" || "$DYNAMIC_SHRINK_POLICY" == "plan" ]]; then
    planned_min_free_mib="${VLLM_ASCEND_MODE1_TRAINING_MIN_FREE_MIB:-28672}"
    if ! [[ "$planned_min_free_mib" =~ ^[0-9]+$ ]]; then
        echo "VLLM_ASCEND_MODE1_TRAINING_MIN_FREE_MIB must be a nonnegative integer: $planned_min_free_mib" >&2
        exit 2
    fi
    if (( planned_min_free_mib < 28672 )); then
        echo "[dynamic length-aware] raising planned training HBM reserve from ${planned_min_free_mib} MiB to 28672 MiB" >&2
        planned_min_free_mib=28672
    fi
...
DYNAMIC_HISTORY_STEPS="${DYNAMIC_HISTORY_STEPS:-$DYNAMIC_PLAN_STEPS}"
...
build_offline_planning_history() {
    local dir="$1"
    local expected_steps="${2:-$DYNAMIC_PLAN_STEPS}"
...
validate_rollout_history() {
    ...
    for dir in $normalized; do
        ...
        validate_rollout_dir "$dir" "$label" "$DYNAMIC_HISTORY_STEPS"
        build_offline_planning_history "$dir" "$DYNAMIC_HISTORY_STEPS"
    done
    IFS="$old_ifs"
    printf '%s' "$normalized"
}
...
run_mode1_epoch() {
    ...
    build_offline_planning_history "$output_dir"
...
echo "[dynamic length-aware] reusable_history_steps=$DYNAMIC_HISTORY_STEPS"
