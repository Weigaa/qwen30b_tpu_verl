...
echo "[mode0 baseline wrapper] mode=${VLLM_ASCEND_ELASTIC_EXECUTION_MODE} floor=${VLLM_ASCEND_ELASTIC_MIN_COMPUTE_GROUP_SIZE} shrink=${VLLM_ASCEND_ENABLE_ELASTIC_PARALLEL_SHRINK} sidecar=${VERL_SIDECAR_ENABLE} custom_models=${VLLM_ASCEND_REGISTER_CUSTOM_MODELS} native_qwen3=${VLLM_ASCEND_USE_NATIVE_QWEN3_MOE} relaxed_ckpt_load=${VLLM_MCORE_DIST_CKPT_RELAXED_LOAD} rollout_gpu_memory_utilization=${ROLLOUT_GPU_MEMORY_UTILIZATION} dataset_fraction=${DATASET_FRACTION:-0.005} data_shuffle=${DATA_SHUFFLE:-True} moe_stats=${VLLM_MOE_PATTERN_STATS} moe_stats_timing=${VLLM_MOE_STATS_TIMING} elastic_util_log=${VLLM_ASCEND_ELASTIC_UTIL_LOG} dummy_waste_timing=${VLLM_ASCEND_DUMMY_WASTE_TIMING} save_rollout_artifacts=${MODE0_SAVE_ROLLOUT_ARTIFACTS:-0} record_dir=${RECORD_DIR}"
