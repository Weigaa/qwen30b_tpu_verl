...
#    - mode=3 不依赖冗余专家；运行时 expert double buffer 按模型 routed expert 数量推导
...
if [[ "${VLLM_ASCEND_ELASTIC_EXECUTION_MODE}" == "0" ]]; then
    # Baseline mode is full-world execution with no shrink. Keep the floor at
    # 16 so run logs do not look like a floor=2 shrink experiment.
...
# mode=3 时该值不控制双缓冲大小；runtime buffer 按模型和最小 floor 推导
