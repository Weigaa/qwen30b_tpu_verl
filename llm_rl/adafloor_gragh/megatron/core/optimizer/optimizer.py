...
class ChainedOptimizer(MegatronOptimizer):
    ...
    def count_zeros(self):
        # The shared grad-stats path used to run a full-parameter ReduceSum even
        # when zero-gradient logging was disabled on every chained optimizer.
        # Besides doing unused work, that reduction can require a large temporary
        # workspace at the actor-training memory peak.
        if not any(optimizer.config.log_num_zeros_in_grad
                   for optimizer in self.chained_optimizers):
            return 0
        if self.grads_states_parallel_group_is_shared():
            params = []
