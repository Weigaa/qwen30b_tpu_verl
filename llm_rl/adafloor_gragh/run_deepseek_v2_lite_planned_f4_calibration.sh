...
CALIBRATION_ROOT=${DEEPSEEK_P_F4_KV_CALIBRATION_ROOT:-/data/adafloor_shared_state/deepseek_v2_lite/kv_calibration_planned_f4_$timestamp}
CAP_ENV=${DEEPSEEK_P_F4_KV_CAP_ENV:-${DEEPSEEK_KV_CAP_ENV:-$SCRIPT_DIR/deepseek_v2_lite_kv_caps.env}}
...
if (( $# != 0 )); then
    echo "DeepSeek Planned floor4 KV calibration does not accept workload overrides" >&2
    exit 2
fi
if ! [[ "${DEEPSEEK_P_F4_TRAINING_MIN_FREE_MIB:-}" =~ ^[1-9][0-9]*$ ]]; then
    echo "DEEPSEEK_P_F4_TRAINING_MIN_FREE_MIB must be an explicitly measured positive integer" >&2
    exit 2
fi

# shellcheck disable=SC1091
source "$RUNTIME_PROFILE_PATH"
IFS=, read -r -a runtime_profile_files <<< "$DEEPSEEK_P_F4_RUNTIME_PROFILE_FILES"
profile_hash_args=()
...
if [[ -e "$CALIBRATION_ROOT" ]]; then
    echo "refusing to overwrite DeepSeek Planned floor4 calibration: $CALIBRATION_ROOT" >&2
    exit 2
fi
...
probe_env=(
    ...
    DEEPSEEK_P_F4_HEADROOM_FLOOR4="$DEEPSEEK_P_F4_HEADROOM_FLOOR4"
    DEEPSEEK_P_F4_HEADROOM_FLOOR8="$DEEPSEEK_P_F4_HEADROOM_FLOOR8"
    DEEPSEEK_P_F4_HEADROOM_FLOOR16="$DEEPSEEK_P_F4_HEADROOM_FLOOR16"
    DEEPSEEK_P_F4_TRAINING_MIN_FREE_MIB="$DEEPSEEK_P_F4_TRAINING_MIN_FREE_MIB"
)

echo "[DeepSeek Planned floor4 calibration] validating common epoch0 protocol"
env "${probe_env[@]}" DEEPSEEK_PROBE_DRY_RUN=1 \
    "$SCRIPT_DIR/run_deepseek_v2_lite_kv_probe.sh" planned_f4 16

mkdir -p "$CALIBRATION_ROOT"
printf '%s\n' "INCOMPLETE DeepSeek Planned floor4 KV calibration" > "$CALIBRATION_ROOT/INCOMPLETE"

for floor in 16 8 4; do
    echo "[DeepSeek Planned floor4 calibration] plan-only preflight floor$floor"
    env "${probe_env[@]}" \
        DEEPSEEK_KV_PROBE_OUTPUT_ROOT="$CALIBRATION_ROOT" \
        DYNAMIC_RUN_NAME="planned_f4_floor${floor}" \
        DEEPSEEK_PROBE_PLAN_ONLY=1 \
        "$SCRIPT_DIR/run_deepseek_v2_lite_kv_probe.sh" planned_f4 "$floor"
done

for floor in 16 8 4; do
    echo "[DeepSeek Planned floor4 calibration] probing floor$floor"
    env "${probe_env[@]}" \
        DEEPSEEK_KV_PROBE_OUTPUT_ROOT="$CALIBRATION_ROOT" \
        DYNAMIC_RUN_NAME="planned_f4_floor${floor}" \
        "$SCRIPT_DIR/run_deepseek_v2_lite_kv_probe.sh" planned_f4 "$floor"
...
python3 "$SCRIPT_DIR/tools/generate_deepseek_kv_caps.py" \
    --lifecycle planned_f4 \
    --common-epoch0-root "$COMMON_EPOCH0_ROOT" \
    --model-path "$MODEL_PATH" \
    --floor4-summary "$CALIBRATION_ROOT/planned_f4_floor4/kv_probe_summary.json" \
    --floor8-summary "$CALIBRATION_ROOT/planned_f4_floor8/kv_probe_summary.json" \
    --floor16-summary "$CALIBRATION_ROOT/planned_f4_floor16/kv_probe_summary.json" \
    --output "$CAP_ENV" \
    --runtime-profile "$DEEPSEEK_P_F4_RUNTIME_PROFILE_ID" \
    ...
    --planned-headroom-floor4 "$DEEPSEEK_P_F4_HEADROOM_FLOOR4" \
    --planned-headroom-floor8 "$DEEPSEEK_P_F4_HEADROOM_FLOOR8" \
    --planned-headroom-floor16 "$DEEPSEEK_P_F4_HEADROOM_FLOOR16" \
    --training-min-free-mib "$DEEPSEEK_P_F4_TRAINING_MIN_FREE_MIB" \
    --target-ratio 1.0 \
    --merge-existing

rm -f "$CALIBRATION_ROOT/INCOMPLETE"
printf '%s\n' \
    "COMPLETE DeepSeek Planned floor4 KV calibration" \
    "COMMON_EPOCH0_ROOT=$COMMON_EPOCH0_ROOT" \
    "PROBE_HISTORY_ROOT=$PROBE_HISTORY_ROOT" \
    "RUNTIME_PROFILE_ID=$DEEPSEEK_P_F4_RUNTIME_PROFILE_ID" \
    "RUNTIME_PROFILE_FILES=$DEEPSEEK_P_F4_RUNTIME_PROFILE_FILES" \
    ...
    "PLANNED_HEADROOM_FLOOR4=$DEEPSEEK_P_F4_HEADROOM_FLOOR4" \
    "PLANNED_HEADROOM_FLOOR8=$DEEPSEEK_P_F4_HEADROOM_FLOOR8" \
    "PLANNED_HEADROOM_FLOOR16=$DEEPSEEK_P_F4_HEADROOM_FLOOR16" \
    "TRAINING_MIN_FREE_MIB=$DEEPSEEK_P_F4_TRAINING_MIN_FREE_MIB" \
    "CAP_ENV=$CAP_ENV" \
    > "$CALIBRATION_ROOT/COMPLETE"

echo "[DeepSeek Planned floor4 calibration] candidate caps=$CAP_ENV"
echo "[DeepSeek Planned floor4 calibration] root=$CALIBRATION_ROOT"
