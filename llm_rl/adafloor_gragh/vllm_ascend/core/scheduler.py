...
class AscendScheduler(Scheduler):
    ...
    def __init__(
        ...
        self.decode_max_num_running_reqs = max(self.max_num_running_reqs,
                                               decode_max_num_seqs)
        self._mode1_kv_diag_seen_batches: set[tuple[str, ...]] = set()

    def _mode1_kv_preempt_diag_enabled(self) -> bool:
        return os.getenv("VLLM_ASCEND_MODE1_KV_PREEMPT_DIAG", "0") == "1"

    def _get_request_block_count(self, request: Request) -> int:
        try:
    ...
    def _format_request_kv_diag(self, request: Request) -> str:
        return (
            f"id={request.request_id},status={request.status.name},"
            f"blocks={self._get_request_block_count(request)},"
            f"computed={request.num_computed_tokens},"
            f"tokens={request.num_tokens},out={request.num_output_tokens},"
            f"max={request.max_tokens},cached={request.num_cached_tokens}")

    def _log_mode1_kv_preempt_diag(
        self,
        phase: str,
        request: Request,
        num_new_tokens: int,
        ...
        if not self._mode1_kv_preempt_diag_enabled():
            return
        block_pool = self.kv_cache_manager.block_pool
        free_blocks = block_pool.get_num_free_blocks()
        total_blocks = getattr(block_pool, "num_gpu_blocks", -1)
        used_blocks = (total_blocks - free_blocks
                       if isinstance(total_blocks, int) else -1)
        limit = int(os.getenv("VLLM_ASCEND_MODE1_KV_PREEMPT_DIAG_LIMIT",
                              "40"))
        running_reqs = [self._format_request_kv_diag(req)
                        for req in self.running[:limit]]
        if len(self.running) > limit:
            running_reqs.append(f"...{len(self.running) - limit} more")
        logger.warning(
            "[mode1_kv_diag] scheduler_%s request={%s} preempted={%s} "
            "num_new_tokens=%s free_blocks=%s used_blocks=%s "
            "total_blocks=%s running=%s waiting=%s scheduled_running=%s "
            "running_reqs=[%s]",
            phase,
            self._format_request_kv_diag(request),
            (self._format_request_kv_diag(preempted_req)
             if preempted_req is not None else "none"),
            num_new_tokens,
            free_blocks,
            used_blocks,
            total_blocks,
    ...
    def _maybe_log_mode1_kv_batch_start(self) -> None:
        if not self._mode1_kv_preempt_diag_enabled() or self.running:
            return
        try:
        ...
        limit = int(os.getenv("VLLM_ASCEND_MODE1_KV_PREEMPT_DIAG_LIMIT",
                              "40"))
        key = tuple(req.request_id for req in waiting_reqs[:limit])
        if key in self._mode1_kv_diag_seen_batches:
            return
        self._mode1_kv_diag_seen_batches.add(key)
        ...
        used_blocks = (total_blocks - free_blocks
                       if isinstance(total_blocks, int) else -1)
        waiting_summary = [self._format_request_kv_diag(req)
                           for req in waiting_reqs[:limit]]
        if len(waiting_reqs) > limit:
            waiting_summary.append(f"...{len(waiting_reqs) - limit} more")
        logger.warning(
            "[mode1_kv_diag] scheduler_batch_start free_blocks=%s "
            "used_blocks=%s total_blocks=%s waiting=%s running=%s "
            "max_num_running_reqs=%s max_num_scheduled_tokens=%s "
            "waiting_reqs=[%s]",
            free_blocks,
            used_blocks,
            total_blocks,
    ...
    def schedule(self) -> SchedulerOutput:
        ...
        self._maybe_log_mode1_kv_batch_start()
            ...
            while req_index < len(self.running) and token_budget > 0:
                ...
                while True:
                    ...
                    if new_blocks is None:
                        ...
                        self._log_mode1_kv_preempt_diag(
                            "alloc_failed_before_preempt",
                            request,
                            num_new_tokens,
                            preempted_req,
                        )
