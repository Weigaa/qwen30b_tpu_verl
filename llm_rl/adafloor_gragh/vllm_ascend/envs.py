...
env_variables: Dict[str, Callable[[], Any]] = {
    ...
    # Keep topology-sensitive elastic MoE execution outside piecewise ACLGraph
    # while retaining graph replay for the surrounding dense subgraphs.
    "VLLM_ASCEND_ELASTIC_ACLGRAPH":
    lambda: bool(int(os.getenv("VLLM_ASCEND_ELASTIC_ACLGRAPH", "0"))),
    # Capture the Ascend attention custom op inside stable PIECEWISE regions.
    # Unsupported batch shapes remain gated by the attention metadata builder;
    # elastic MoE stays an explicit topology-sensitive boundary.
    "VLLM_ASCEND_ELASTIC_ACLGRAPH_CAPTURE_ATTENTION":
    lambda: bool(
        int(os.getenv("VLLM_ASCEND_ELASTIC_ACLGRAPH_CAPTURE_ATTENTION", "0"))),
    # Capture the elastic MoE custom op and its current topology-specific
    # dispatch/compute/combine sequence inside PIECEWISE ACLGraph. The graph
    # lifecycle must invalidate and recapture whenever the active floor,
    # expert layout, communicator set, weights, or KV allocation changes.
    "VLLM_ASCEND_ELASTIC_ACLGRAPH_CAPTURE_MOE":
    lambda: bool(
        int(os.getenv("VLLM_ASCEND_ELASTIC_ACLGRAPH_CAPTURE_MOE", "0"))),
    # PyTorch-ABI-compatible vLLM Ascend extension used by elastic ACLGraph.
    # The copied 0.11 runtime can carry an older extension, so graph mode pins
    # and hashes the compatible library before Ray workers are launched.
    "VLLM_ASCEND_ELASTIC_ACLGRAPH_EXTENSION":
    lambda: os.getenv("VLLM_ASCEND_ELASTIC_ACLGRAPH_EXTENSION", ""),
    "VLLM_ASCEND_ELASTIC_ACLGRAPH_EXTENSION_SHA256":
    lambda: os.getenv("VLLM_ASCEND_ELASTIC_ACLGRAPH_EXTENSION_SHA256", ""),
