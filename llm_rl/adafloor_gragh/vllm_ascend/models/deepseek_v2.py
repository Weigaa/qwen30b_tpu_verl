...
class CustomDeepseekV2DecoderLayer(DeepseekV2DecoderLayer):
    ...
    def __init__(
        ...
        nn.Module.__init__(self)
        # vLLM passes this buffer for DeepSeek V3.2 sparse attention. The
        # DeepSeek-V2 path has no sparse index buffer, but its decoder must
        # retain the common constructor signature used by DeepseekV2Model.
        if topk_indices_buffer is not None:
            raise NotImplementedError(
                "Ascend sparse-index attention is not supported by this "
                "DeepSeek decoder"
            )
