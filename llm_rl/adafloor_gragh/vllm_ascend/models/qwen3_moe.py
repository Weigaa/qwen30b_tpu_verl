...
class CustomSparseMoeBlock(Qwen3MoeSparseMoeBlock):
    ...
    def __init__(
        ...
        prefix: str = "",
        use_elastic_aclgraph: bool = False,
    ):
        nn.Module.__init__(self)
        self.tp_size = get_tensor_model_parallel_world_size()
        self.prefix = prefix
        self.use_elastic_aclgraph = use_elastic_aclgraph
    ...
    def forward(
        ...
        if self.use_elastic_aclgraph:
            hidden_states = torch.ops.vllm.elastic_ascend_moe_forward(
...
class CustomQwen3MoeDecoderLayer(Qwen3MoeDecoderLayer):
    ...
    def __init__(
        ...
        self.use_elastic_aclgraph = (
            self.use_aclgraph
            and envs_ascend.VLLM_ASCEND_ELASTIC_ACLGRAPH)
        if (layer_idx not in mlp_only_layers) and (
                config.num_experts > 0 and
            (layer_idx + 1) % config.decoder_sparse_step == 0):
            if not self.use_aclgraph or self.use_elastic_aclgraph:
                self.mlp = CustomSparseMoeBlock(config=config,
                                                quant_config=quant_config,
                                                prefix=f"{prefix}.mlp",
                                                use_elastic_aclgraph=
                                                self.use_elastic_aclgraph)
            else:
