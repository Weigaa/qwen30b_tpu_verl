...
def weak_ref_tensor(tensor: Any) -> Any:
    ...
    if isinstance(tensor, torch.Tensor):
        ensure_ascend_weak_ref_tensor_op()
        return torch.ops._C_ascend.weak_ref_tensor(tensor)
    else:
        return tensor


def ensure_ascend_weak_ref_tensor_op() -> None:
    """Load the pinned current-PyTorch extension required by ACLGraph.

    Some copied vLLM-Ascend 0.11 trees include a C++ extension built against
    an older PyTorch C++ ABI. Eager execution can tolerate that extension not
    loading, while ACLGraph requires its non-owning output tensor alias op.
    Never approximate that alias with a strong Python tensor reference because
    doing so retains every captured subgraph output and can exhaust HBM.
    """
    if hasattr(torch.ops._C_ascend, "weak_ref_tensor"):
        return

    with _WEAK_REF_OP_LOAD_LOCK:
        if hasattr(torch.ops._C_ascend, "weak_ref_tensor"):
            return

        extension_path = envs_ascend.VLLM_ASCEND_ELASTIC_ACLGRAPH_EXTENSION
        expected_sha256 = (
            envs_ascend.VLLM_ASCEND_ELASTIC_ACLGRAPH_EXTENSION_SHA256)
        if not extension_path or not expected_sha256:
            raise RuntimeError(
                "ACLGraph requires a PyTorch-compatible vllm_ascend_C "
                "extension. Set VLLM_ASCEND_ELASTIC_ACLGRAPH_EXTENSION and "
                "VLLM_ASCEND_ELASTIC_ACLGRAPH_EXTENSION_SHA256 via the "
                "AdaFloor ACLGraph launcher.")
        if not os.path.isabs(extension_path) or not os.path.isfile(
                extension_path):
            raise RuntimeError(
                "Invalid VLLM_ASCEND_ELASTIC_ACLGRAPH_EXTENSION: "
                f"{extension_path!r}")

        digest = hashlib.sha256()
        with open(extension_path, "rb") as extension_file:
            for chunk in iter(lambda: extension_file.read(1024 * 1024), b""):
                digest.update(chunk)
        actual_sha256 = digest.hexdigest()
        if actual_sha256 != expected_sha256:
            raise RuntimeError(
                "vLLM Ascend extension SHA256 mismatch: expected "
                f"{expected_sha256}, got {actual_sha256}")

        try:
            torch.ops.load_library(extension_path)
        except Exception as exc:
            raise RuntimeError(
                "Failed to load the pinned PyTorch-compatible vLLM Ascend "
                f"extension {extension_path}: {exc}") from exc
        if not hasattr(torch.ops._C_ascend, "weak_ref_tensor"):
            raise RuntimeError(
                "Pinned vLLM Ascend extension did not register "
                "_C_ascend.weak_ref_tensor")
        logger.info_once(
            "Loaded ACLGraph weak-ref compatibility extension %s sha256=%s",
            extension_path,
            actual_sha256,
        )
