...
class NPUPlatform(Platform):
    ...
    def check_and_update_config(cls, vllm_config: VllmConfig) -> None:
        ...
        if (envs_ascend.VLLM_ASCEND_ELASTIC_ACLGRAPH
            ...
            raise ValueError(
                "VLLM_ASCEND_ELASTIC_ACLGRAPH requires native PIECEWISE "
                "ACLGraph; set VLLM_ENABLE_GRAPH_MODE=0 to disable TorchAir")
        ...
        capture_elastic_attention = (
            elastic_aclgraph
            and envs_ascend.VLLM_ASCEND_ELASTIC_ACLGRAPH_CAPTURE_ATTENTION)
        capture_elastic_moe = (
            elastic_aclgraph
            and envs_ascend.VLLM_ASCEND_ELASTIC_ACLGRAPH_CAPTURE_MOE)
        if (elastic_aclgraph and compilation_config.cudagraph_mode
                == CUDAGraphMode.PIECEWISE):
            # vLLM 0.11 defaults this to False, which makes PIECEWISE replay
            # reuse the first decode iteration's symbolic input addresses.
            # Copy live symbolic inputs into compiler-owned static buffers
            # before every replay, matching ACLGraph's fixed-address contract.
            compilation_config.cudagraph_copy_inputs = True
        elif (elastic_aclgraph and compilation_config.cudagraph_mode
              == CUDAGraphMode.FULL_DECODE_ONLY):
            # FULL_DECODE_ONLY uses model-runner-owned static input buffers,
            # matching the 0.14 native full-graph contract.  Per-token input
            # copies erase much of the benefit and can hide stale addresses,
            # so the wrapper validates addresses and fails closed instead.
            compilation_config.cudagraph_copy_inputs = False
        if (capture_elastic_attention and compilation_config.cudagraph_mode !=
                CUDAGraphMode.FULL_DECODE_ONLY):
            if compilation_config.cudagraph_mode != CUDAGraphMode.PIECEWISE:
                logger.warning(
                    "Elastic Attention ACLGraph on vLLM-Ascend 0.11 requires "
                    "PIECEWISE mode; overriding %s. Attention is captured "
                    "with dynamic task metadata while elastic MoE/HCCL "
                    "remains a graph boundary.",
                    compilation_config.cudagraph_mode)
                compilation_config.cudagraph_mode = CUDAGraphMode.PIECEWISE
        elif (elastic_aclgraph and compilation_config.cudagraph_mode not in (
                CUDAGraphMode.PIECEWISE,
                CUDAGraphMode.FULL_DECODE_ONLY,
        )):
            logger.warning(
                "Elastic MoE-only ACLGraph requires PIECEWISE mode; "
                "overriding %s. Topology-sensitive MoE is %s ACLGraph.",
                compilation_config.cudagraph_mode,
                "captured inside" if capture_elastic_moe else "kept outside")
            compilation_config.cudagraph_mode = CUDAGraphMode.PIECEWISE
        ...
        # after MLA being supported
            ...
            ascend_attention_op = "vllm.unified_ascend_attention_with_output"
            splitting_ops = ["vllm.mla_forward"]
            if capture_elastic_attention:
                # Keep the unified op outside the FX pieces so its dynamic KV
                # mutation always executes. The op internally captures only
                # the side-effect-free paged-attention read core.
                splitting_ops.insert(0, ascend_attention_op)
            else:
                splitting_ops.insert(0, ascend_attention_op)
            if capture_elastic_attention or (elastic_aclgraph
                                             and not capture_elastic_moe):
                splitting_ops.append("vllm.elastic_ascend_moe_forward")
                logger.info(
                    "Elastic ACLGraph boundary enabled: elastic Ascend MoE/HCCL "
                    "executes outside PIECEWISE ACLGraph; Attention's dynamic "
                    "KV write remains outside and its paged read core executes %s",
                    "in a nested ACLGraph"
                    if capture_elastic_attention else "eagerly")
            elif capture_elastic_moe:
                elastic_moe_op = "vllm.elastic_ascend_moe_forward"
                compilation_config.splitting_ops = [
                    op for op in compilation_config.splitting_ops
                    if op != elastic_moe_op
                ]
                logger.info(
                    "Elastic ACLGraph MoE capture enabled; topology-specific "
                    "dispatch, expert compute, and combine execute inside "
                    "PIECEWISE ACLGraph")
            ...
            logger.info(
                ...
                "using only ACL Graph mode")
            if capture_elastic_attention:
                full_decode_attention_backend = os.getenv(
                    "VLLM_ASCEND_FULL_DECODE_ATTENTION_BACKEND",
                    "fia_max_workspace",
                ).lower()
                if full_decode_attention_backend not in (
                        "fia_max_workspace", "paged_attention"):
                    raise ValueError(
                        "Unsupported FULL_DECODE_ONLY Attention backend: "
                        f"{full_decode_attention_backend!r}")
                logger.info(
                    "Native FULL_DECODE_ONLY ACLGraph enabled: KV write, "
                    "Attention read, MoE, and dense decode execute in the "
                    "outer full graph; live Attention task metadata is "
                    "updated with the vLLM-Ascend 0.14 protocol; "
                    "attention_backend=%s cudagraph_copy_inputs=false",
                    full_decode_attention_backend)
            compilation_config.use_inductor = False
