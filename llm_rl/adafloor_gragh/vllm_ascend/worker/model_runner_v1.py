...
# `_npu_paged_attention_get_workspace` returns the maximum reusable paged-
# attention workspace for this sequence-length bucket. FULL decode graphs use
# it only while capturing; live inference continues to publish the real context
# lengths through the graph-task update protocol. This matches the native
# vLLM-Ascend 0.14 FULL_DECODE_ONLY implementation.
...
def _custom_mode1_kv_alloc_trace_enabled() -> bool:
    return _env_flag("VLLM_ASCEND_MODE1_KV_ALLOC_TRACE", "0")


def _custom_mode1_kv_memory_snapshot() -> dict[str, int]:
...
def _log_custom_mode1_kv_memory(tag: str) -> None:
    ...
    if int(getattr(envs_ascend, "VLLM_ASCEND_ELASTIC_EXECUTION_MODE", 0)) != 1:
        return
    try:
        snapshot = _custom_mode1_kv_memory_snapshot()
        logger.info(
...
def _log_custom_mode1_kv_alloc_event(
        tag: str,
        layer_name: str,
        requested_bytes: int,
        shared_by: object,
    ...
    if int(getattr(envs_ascend, "VLLM_ASCEND_ELASTIC_EXECUTION_MODE", 0)) != 1:
        return
    if exc is None and not _custom_mode1_kv_alloc_trace_enabled():
        return
    try:
        snapshot = _custom_mode1_kv_memory_snapshot()
    except Exception:
        snapshot = {}
    log_fn = logger.exception if exc is not None else logger.info
    log_fn(
        "Custom mode=1 KV alloc: tag=%s layer=%s requested_bytes=%s "
        "shared_by=%s snapshot=%s%s",
        tag,
        layer_name,
        int(requested_bytes),
        shared_by,
        snapshot,
        "" if exc is None else f" error={exc!r}",
    )
...
def _mode1_mc2_warmup_route_bounds(
        warmup_route: str, dispatch_num_experts: int, dispatcher_ep_size: int,
        dispatcher_ep_rank: int, top_k: int) -> tuple[int, int, str]:
    """Choose a valid synthetic route for the post-restore MC2 warmup."""
    warmup_route = str(warmup_route).strip().lower()
    dispatch_num_experts = max(1, int(dispatch_num_experts))
    dispatcher_ep_size = max(1, int(dispatcher_ep_size))
    dispatcher_ep_rank = int(dispatcher_ep_rank)
    top_k = max(1, int(top_k))
    if dispatch_num_experts < top_k:
        raise ValueError(
            "MC2 warmup requires at least top_k distinct global experts: "
            f"experts={dispatch_num_experts} top_k={top_k}")
    if warmup_route in ("global", "full", "all"):
        return 0, dispatch_num_experts, warmup_route

    per_rank = max(1, dispatch_num_experts // dispatcher_ep_size)
    if per_rank < top_k:
        return 0, dispatch_num_experts, f"{warmup_route}_global_fallback"

    route_rank = dispatcher_ep_rank
    if warmup_route in ("next", "remote", "next_rank"):
        route_rank = (dispatcher_ep_rank + 1) % dispatcher_ep_size
    route_base = max(
        0, min(dispatch_num_experts - 1, route_rank * per_rank))
    route_count = max(
        1, min(per_rank, dispatch_num_experts - route_base))
    return route_base, route_count, warmup_route
...
class NPUModelRunner(LoRAModelRunnerMixin):
    ...
    def __init__(self, vllm_config: VllmConfig, device: torch.device):
        ...
        self.num_draft_tokens = self._make_buffer(self.max_num_reqs,
                                                  dtype=torch.int32)
        #extra variables
        self.decode_dummy_time = 0
        self.need_allreduce = True
    ...
    def _prepare_inputs(
        ...
        if self.need_allreduce:
            ...
            # if maybe_padded_num_tokens == 16:
            #     self.need_allreduce = False
        else:
    ...
    def _build_attention_metadata(self, create_mixed_batch, num_reqs,
        ...
        if force_attention:
            ...
            if create_mixed_batch:
                ...
                # Capture PA with its maximum workspace bucket so every replay
                # has sufficient stable workspace. Outside capture, use the
                # real query length; live decode metadata replaces this dummy
                # value before each graph-task update.
                seq_lens = (SEQ_LEN_WITH_MAX_PA_WORKSPACE
                            if is_graph_capturing else max_query_len)
    ...
    def _dummy_run(
        ...
        #         "is_torchair_compile:", is_torchair_compile)
        #     self.decode_dummy_time += 1
        #     print("rank", self.dp_rank, "decode_dummy_time:", self.decode_dummy_time)
        # Padding for DP
        ...
        if self.need_allreduce:
            (num_tokens, num_tokens_across_dp, with_prefill,
            _) = self._sync_metadata_across_dp(num_tokens, with_prefill, False)
            # if num_tokens == 16 and before_num_tokens == 1:
            #     self.need_allreduce = False
        else:
            num_tokens = 16
            num_tokens_across_dp = torch.tensor([16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16])
        # if before_num_tokens == 1:
        ...
        with self.maybe_dummy_run_with_lora(self.lora_config,
            ...
            if self.is_multimodal_model:
                ...
                inputs_embeds = None
                #check dummy input_ids
                # if before_num_tokens == 1:
                #     print("rank", self.dp_rank, "dummy run input_ids:", input_ids)
    ...
    def set_in_profile_run(self):
        old_env = os.environ.get("VLLM_ASCEND_MODE1_IN_PROFILE_RUN")
        self.in_profile_run = True
        if int(getattr(envs_ascend, "VLLM_ASCEND_ELASTIC_EXECUTION_MODE", 0)) == 1:
            os.environ["VLLM_ASCEND_MODE1_IN_PROFILE_RUN"] = "1"
        try:
            yield
        finally:
            self.in_profile_run = False
            if old_env is None:
                os.environ.pop("VLLM_ASCEND_MODE1_IN_PROFILE_RUN", None)
            else:
                os.environ["VLLM_ASCEND_MODE1_IN_PROFILE_RUN"] = old_env
    ...
    def profile_run(self) -> None:
        ...
        with self.set_in_profile_run():
            ...
            original_profile_num_tokens = profile_num_tokens
            if mode1_profile:
                mode1_profile_max_tokens = _env_int(
                    "VLLM_ASCEND_MODE1_PROFILE_MAX_TOKENS",
                    profile_num_tokens,
                    minimum=1,
                )
                profile_num_tokens = min(profile_num_tokens,
                                         mode1_profile_max_tokens)
                if (_env_flag("VLLM_ASCEND_MODE1_PROFILE_AVOID_MC2_BOUNDARY",
                              "1")
                        and profile_num_tokens <= int(self.mc2_tokens_capacity)
                        and original_profile_num_tokens
                        > int(self.mc2_tokens_capacity)):
                    mc2_boundary_avoid_tokens = _env_int(
                        "VLLM_ASCEND_MODE1_PROFILE_MC2_AVOID_TOKENS",
                        int(self.mc2_tokens_capacity) + 128,
                        minimum=int(self.mc2_tokens_capacity) + 1,
                    )
                    adjusted_profile_num_tokens = min(
                        original_profile_num_tokens,
                        max(profile_num_tokens, mc2_boundary_avoid_tokens),
                    )
                    if adjusted_profile_num_tokens != profile_num_tokens:
                        logger.info(
                            "Mode1 profile token cap adjusted away from MC2 "
                            "boundary: original=%s cap=%s mc2_capacity=%s "
                            "avoid_tokens=%s effective=%s",
                            original_profile_num_tokens,
                            mode1_profile_max_tokens,
                            self.mc2_tokens_capacity,
                            mc2_boundary_avoid_tokens,
                            adjusted_profile_num_tokens,
                        )
                        profile_num_tokens = adjusted_profile_num_tokens
                if profile_num_tokens != original_profile_num_tokens:
                    logger.info(
                        "Mode1 profile token cap applied: original=%s cap=%s "
                        "effective=%s",
                        original_profile_num_tokens,
                        mode1_profile_max_tokens,
                        profile_num_tokens,
                    ...
                    logger.info(
                        "Mode1 profile token cap not applied: tokens=%s cap=%s",
                        profile_num_tokens,
                        mode1_profile_max_tokens,
                    )
                expected_profile_comm = os.getenv(
                    "VLLM_ASCEND_MODE1_PROFILE_EXPECT_MOE_COMM", ""
                ).strip().upper()
                if expected_profile_comm:
                    selected_profile_comm = self._select_moe_comm_method(
                        profile_num_tokens, with_prefill=True
                    )
                    selected_profile_comm_name = selected_profile_comm.name.upper()
                    normalized_expected = expected_profile_comm.replace("-", "").replace(
                        "_", ""
                    )
                    normalized_selected = selected_profile_comm_name.replace(
                        "_", ""
                    )
                    if normalized_expected != normalized_selected:
                        raise RuntimeError(
                            "Mode1 profile MoE communication contract mismatch: "
                            f"tokens={profile_num_tokens} expected="
                            f"{expected_profile_comm} selected="
                            f"{selected_profile_comm_name} mc2_capacity="
                            f"{self.mc2_tokens_capacity}"
                        )
                    logger.info(
                        "Mode1 profile MoE communication contract satisfied: "
                        "tokens=%s selected=%s mc2_capacity=%s",
                        profile_num_tokens,
                        selected_profile_comm_name,
                        self.mc2_tokens_capacity,
                    )
            ...
            #
            # Mode1 manages MC2/EP shrink resources explicitly and may size KV
            # cache against a low floor before real rollout. A second synthetic
            # 512-token MC2 profile can request a large HCCL dispatch workspace
            # (~1.56 GiB on A3) before the mode1 lifecycle has a chance to apply
            # its planned warmup/cleanup. Keep the native behavior outside mode1,
            # but make this extra profile bounded or skippable under mode1.
            run_extra_mc2_profile = True
            mc2_profile_tokens = int(self.mc2_tokens_capacity)
            if mode1_profile:
                run_extra_mc2_profile = _env_flag(
                    "VLLM_ASCEND_MODE1_PROFILE_EXTRA_MC2_DUMMY", "0")
                mc2_profile_tokens = min(
                    mc2_profile_tokens,
                    _env_int("VLLM_ASCEND_MODE1_PROFILE_MC2_DUMMY_TOKENS",
                             mc2_profile_tokens,
                             minimum=1),
                )
            if run_extra_mc2_profile and mc2_profile_tokens > 0:
                logger.info(
                    "Profile MC2 boundary dummy run: mode1=%s tokens=%s "
                    "capacity=%s profile_tokens=%s",
                    mode1_profile,
                    mc2_profile_tokens,
                    self.mc2_tokens_capacity,
                    profile_num_tokens,
                )
                self._dummy_run(mc2_profile_tokens, with_prefill=True)
            elif mode1_profile:
                logger.info(
                    "Profile MC2 boundary dummy run skipped for mode1: "
                    "capacity=%s profile_tokens=%s",
                    self.mc2_tokens_capacity,
                    profile_num_tokens,
                )
    ...
    def warmup_mode1_parity_mc2_dispatcher_only(self,
        ...
        with set_ascend_forward_context(
            ...
            warmup_route = os.getenv(
                "VLLM_ASCEND_MODE1_PARITY_MC2_WARMUP_ROUTE",
                "local").strip().lower()
            dispatcher_ep_size = max(
                1, int(getattr(token_dispatcher, "ep_world_size", 1)))
            dispatcher_ep_rank = int(getattr(token_dispatcher, "ep_rank_id", 0))
            requested_warmup_route = warmup_route
            route_base, route_count, warmup_route = (
                _mode1_mc2_warmup_route_bounds(
                    warmup_route,
                    dispatch_num_experts,
                    dispatcher_ep_size,
                    dispatcher_ep_rank,
            ...
            if warmup_route != requested_warmup_route:
                logger.warning(
                    "Mode1 parity MC2 warmup route fallback: rank=%s "
                    "requested=%s effective=%s local_experts=%s top_k=%s",
                    rank,
                    requested_warmup_route,
                    warmup_route,
                    int(dispatch_num_experts) // dispatcher_ep_size,
                    top_k,
                )
            topk_ids = (
                route_base +
                (torch.arange(num_tokens * top_k,
                              dtype=torch.int32,
                              device=self.device) % route_count)).reshape(
                                  num_tokens, top_k).contiguous()
            logger.info(
                "Mode1 parity MC2 dispatcher-only warmup route: rank=%s "
                "route=%s ep_rank=%s ep_size=%s dispatch_num_experts=%s "
                "route_base=%s route_count=%s topk_min=%s topk_max=%s "
                "topk_unique=%s",
                rank,
                warmup_route,
                dispatcher_ep_rank,
                ...
                route_base,
                route_count,
                int(topk_ids.min().item()) if topk_ids.numel() else -1,
                int(topk_ids.max().item()) if topk_ids.numel() else -1,
                int(torch.unique(topk_ids).numel()) if topk_ids.numel() else 0,
            )
            try:
                dispatch_results = token_dispatcher.token_dispatch(
                    hidden_states=hidden_states,
    ...
    def load_model(self) -> None:
        ...
        # Full graphs own this state through their outer wrapper. The 0.11
        # Attention-in-PIECEWISE port also needs one update stream and task map.
        if (self.compilation_config.cudagraph_mode.has_full_cudagraphs()
                or envs_ascend.VLLM_ASCEND_ELASTIC_ACLGRAPH_CAPTURE_ATTENTION):
            ...
            for idx in range(len(kv_cache_tensor.shared_by)):
                ...
                if "linear_attn" in layer_name:
                    ...
                    for layer_name_inner in kv_cache_tensor.shared_by:
                        ...
                        if self.vllm_config.kv_transfer_config is None:
                            try:
                                _log_custom_mode1_kv_alloc_event(
                                    "before_linear_attn_tensor_alloc",
                                    layer_name_inner,
                                    int(kv_cache_tensor.size),
                                ...
                                _log_custom_mode1_kv_alloc_event(
                                    "failed_linear_attn_tensor_alloc",
                                    layer_name_inner,
                                    int(kv_cache_tensor.size),
                                    kv_cache_tensor.shared_by,
                                    exc,
                            ...
                            try:
                                _log_custom_mode1_kv_alloc_event(
                                    "before_linear_attn_tensor_alloc_aligned",
                                    layer_name_inner,
                                    int(cache_size_aligned),
                                    kv_cache_tensor.shared_by,
                                )
                                ...
                                _log_custom_mode1_kv_alloc_event(
                                    "failed_linear_attn_tensor_alloc_aligned",
                                    layer_name_inner,
                                    int(cache_size_aligned),
                                    kv_cache_tensor.shared_by,
                                    exc,
                    ...
                    if self.vllm_config.kv_transfer_config is None:
                        cache_size = int(kv_cache_tensor.size // 2)
                        try:
                            _log_custom_mode1_kv_alloc_event(
                                "before_k_tensor_alloc",
                                layer_name,
                                cache_size,
                                kv_cache_tensor.shared_by,
                            )
                            k_tensor = torch.zeros(cache_size,
                                                   dtype=torch.int8,
                                                   device=self.device)
                            _log_custom_mode1_kv_alloc_event(
                                "before_v_tensor_alloc",
                                layer_name,
                                cache_size,
                                kv_cache_tensor.shared_by,
                            )
                            v_tensor = torch.zeros(cache_size,
                                                   dtype=torch.int8,
                                                   device=self.device)
                        except RuntimeError as exc:
                            _log_custom_mode1_kv_alloc_event(
                                "failed_kv_tensor_alloc",
                                layer_name,
                                cache_size,
                                kv_cache_tensor.shared_by,
                                exc,
                        ...
                        try:
                            ...
                            _log_custom_mode1_kv_alloc_event(
                                "failed_kv_tensor_alloc_aligned",
                                layer_name,
                                int(cache_size_aligned),
                                kv_cache_tensor.shared_by,
                                exc,
    ...
    def _capture_aclgraphs(self, compilation_cases: list[int],
        ...
        for num_tokens in compilation_cases:
            for _ in range(self.compilation_config.cudagraph_num_of_warmups):
                ...
                force_attention = (
                    aclgraph_runtime_mode == CUDAGraphMode.FULL
                    or (aclgraph_runtime_mode == CUDAGraphMode.PIECEWISE
                        and envs_ascend.
    ...
    @contextmanager
    def _isolate_live_kv_during_aclgraph_capture(self):
        """Prevent a mid-rollout graph recapture from mutating live KV slots.

        FULL_DECODE_ONLY captures Attention's reshape-and-cache operation. A
        topology change can trigger capture while requests are still active,
        so the persistent slot-mapping buffer may still name their live KV
        slots. Capture must retain that buffer's address for replay, but its
        dummy forward must not write dummy key/value tensors to those slots.
        PAD_SLOT_ID preserves the address while making the capture-time KV
        write a no-op. Runtime metadata restores real slot IDs before replay.
        """
        captures_attention = (
            self.use_aclgraph
        ...
        torch.npu.synchronize()
        saved_slot_mapping = self.slot_mapping.clone()
        self.slot_mapping.fill_(PAD_SLOT_ID)
        torch.npu.synchronize()
        try:
            yield
        finally:
            # Capture runs on a dedicated stream. Wait for it before changing
            # the input contents that the recorded graph will read on replay.
            torch.npu.synchronize()
            self.slot_mapping.copy_(saved_slot_mapping)
            torch.npu.synchronize()
        logger.info(
            "ACLGraph capture isolated live KV writes with PAD_SLOT_ID=%s",
            PAD_SLOT_ID,
        )
    ...
    def capture_model(self) -> None:
        ...
        with self._isolate_live_kv_during_aclgraph_capture():
            self._capture_model()
