...
from verl.utils.device import get_device_id, is_npu_available
...
def _log_custom_mode1_rollout_memory(tag: str) -> None:
    ...
    try:
        ...
        logger.exception("Failed to log custom mode=1 rollout memory: tag=%s",
                         tag)
...
def _materialize_rollout_weight_staging(
    ...
    The mode=1 step2 OOM regression shows that after `update_weights_load`,
    torch-visible allocations can remain about 8+ GB above the known-good
    plateau. One practical failure mode is that rollout reload keeps
    caller-owned tensors alive through loader-side references / aliases longer
    than intended. Cloning into rollout-owned staging tensors cuts that tie.
    """
...
class vLLMRollout(BaseRollout):
    def __init__(
        ...
        if keep_engine_loaded:
            logger.info(
                "Keep rollout engine loaded after init "
                "(VERL_STANDALONE_ROLLOUT_KEEP_ENGINE_LOADED=1)"
            )
        else:
    ...
    def onload_model_weights(self):
        ...
        for name, param in self.model.named_parameters():
            ...
            if target_format is not None and str(target_format) not in ("ND", "0"):
                try:
                    import torch_npu  # type: ignore
    ...
    def free_cache_engine(self):
        ...
        for layer_name in layer_need_kv_cache:
            ...
            # The layer modules themselves can also hold strong kv_cache
            # references; clear those as well so the second resume lifecycle
            # does not retain the previous large cache tensors.
            try:
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        # 核心运行引擎调用
        use_tqdm = os.getenv("VERL_ROLLOUT_USE_TQDM", "1").strip().lower() in ("1", "true", "yes", "on")
    ...
    def _iter_hf_safetensor_weights(self, model_path: str) -> Generator[tuple[str, torch.Tensor], None, None]:
        ...
        if index_path.exists():
            ...
            if not single_path.exists():
                raise FileNotFoundError(f"Cannot find safetensors weights under {model_dir}")
            with safe_open(str(single_path), framework="pt", device="cpu") as f:
                shard_to_keys = {"model.safetensors": list(f.keys())}
        ...
        for shard_name in sorted(shard_to_keys):
            ...
            with safe_open(str(shard_path), framework="pt", device="cpu") as f:
                for key in shard_to_keys[shard_name]:
                    yield key, f.get_tensor(key).to(device, non_blocking=True)

    async def update_weights_from_hf(self, model_path: str):
        from verl.utils.vllm.patch import patch_vllm_moe_model_weight_loader

        model = self.inference_engine.llm_engine.model_executor.driver_worker.worker.model_runner.get_model()
        patch_vllm_moe_model_weight_loader(model)
        loaded_params = model.load_weights(self._iter_hf_safetensor_weights(model_path))
        logger.warning(
            "vLLM load HF weights directly from safetensors, loaded_params: %s model_path=%s",
