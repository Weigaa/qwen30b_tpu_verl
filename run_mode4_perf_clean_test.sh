...
usage() {
    cat <<'EOF'
Usage:
  ./run_mode4_perf_clean_test.sh [floor]
  ./run_mode4_perf_clean_test.sh --floor 1

Environment overrides:
  MODE4_FLOOR=1
  REPO_ROOT=/path/to/log/output/root
  PATCH_TREE=/path/to/code/tree
  HCCL_IF_BASE_PORT=46841
  MASTER_PORT=25840
  VERL_HCCL_IF_BASE_PORT_START=46841

Examples:
  ./run_mode4_perf_clean_test.sh
  ./run_mode4_perf_clean_test.sh 1
  ./run_mode4_perf_clean_test.sh --floor 4
EOF
}
...
case "$floor" in
    1|2|4|8|16) ;;
    *)
        echo "unsupported mode=4 floor: $floor; expected one of 1,2,4,8,16" >&2
        exit 2
        ;;
esac
...
tee_log="$REPO_ROOT/mode4_floor${floor}_perf_clean_${stamp}.log"

# Performance run: disable profiler, stage-decode MSTX markers, and per-layer
# timing sync/logging.
export VLLM_ASCEND_ELASTIC_EXECUTION_MODE=4
...
' "$tee_log"
printf '[mode4 perf clean] floor=%s runtime_floor=%s\n' "$floor" "$VLLM_ASCEND_MODE4_RUNTIME_MIN_COMPUTE_GROUP_SIZE"
printf '[mode4 perf clean] profile=%s timing_log=%s timing_sync=%s markers=%s
