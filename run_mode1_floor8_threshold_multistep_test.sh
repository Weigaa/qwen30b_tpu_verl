...
usage() {
    cat <<'EOF'
Usage:
  bash run_mode1_floor8_threshold_multistep_test.sh [floor]
  bash run_mode1_floor8_threshold_multistep_test.sh --floor 8
...
# Keep the same KV cap as the floor=4 reference run so this script only
# changes the minimum elastic compute group size.
