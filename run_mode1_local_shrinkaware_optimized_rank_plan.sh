...
usage() {
    cat <<'EOF'
Usage:
  ./run_mode1_local_shrinkaware_optimized_rank_plan.sh [extra hydra args...]

Mode=1 floor=4 shrink-aware optimized-rank-plan experiment:
  1. Reads baseline rollout responses from mode1_baseline_random_batch_floor4.
  2. Solves a 5-step/16-rank assignment problem using baseline max response
     length for staged completion order and active-token peak for preemption risk.
  3. Writes an optimized train parquet and exact rank plan.
  4. Runs rollout with shrink-aware staged 16->8->4 and the optimized plan.
EOF
}
...
export OUTPUT_SUBDIR="${OUTPUT_SUBDIR:-mode1_shrinkaware_optimized_rank_plan_floor4}"
...
MAX_RANK_PEAK_TOKENS="${MAX_RANK_PEAK_TOKENS:-$VLLM_ASCEND_MODE1_PARITY_MAX_KV_TOKENS_FLOOR4}"

PLAN_DIR="${PLAN_DIR:-$REPO_ROOT/mode1_shrinkaware_optimized_rank_plan_floor4/oracle}"
OPT_TRAIN_FILE="${OPT_TRAIN_FILE:-$PLAN_DIR/optimized_train.parquet}"
OPT_RANK_PLAN="${OPT_RANK_PLAN:-$PLAN_DIR/optimized_rank_plan.json}"
OPT_SUMMARY="${OPT_SUMMARY:-$PLAN_DIR/optimized_rank_plan_summary.json}"
OPT_ORACLE="${OPT_ORACLE:-$PLAN_DIR/optimized_length_oracle.json}"
mkdir -p "$PLAN_DIR"

python3 "$PATCH_TREE/tools/build_mode1_optimized_rank_plan.py" \
...
tee_log="$REPO_ROOT/mode1_local_shrinkaware_optimized_rank_plan_floor4_${stamp}.log"

printf '[mode1 shrink-aware optimized rank plan] runtime_cwd=%s\n' "$PATCH_TREE"
printf '[mode1 shrink-aware optimized rank plan] launcher=%s tee_log=%s\n' "$LAUNCHER" "$tee_log"
printf '[mode1 shrink-aware optimized rank plan] train=%s plan=%s summary=%s oracle=%s baseline=%s\n' \
    "$TRAIN_FILE" "$OPT_RANK_PLAN" "$OPT_SUMMARY" "$OPT_ORACLE" "$BASELINE_DIR"
printf '[mode1 shrink-aware optimized rank plan] model=%s distcp=%s test=%s\n' \
    "$MODEL_PATH" "$DISTCP_PATH" "$TEST_FILE"
printf '[mode1 shrink-aware optimized rank plan] mode=1 floor=4 util=%s kv_cap_floor4=%s total_epochs=%s\n' \
    "$ROLLOUT_GPU_MEMORY_UTILIZATION" \
    "$VLLM_ASCEND_MODE1_PARITY_MAX_KV_TOKENS_FLOOR4" \
    "$TRAINER_TOTAL_EPOCHS"
printf '[mode1 shrink-aware optimized rank plan] max_rank_peak_tokens=%s\n' "$MAX_RANK_PEAK_TOKENS"
