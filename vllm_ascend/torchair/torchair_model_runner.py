...
class NPUTorchairModelRunner(NPUModelRunner):
    ...
    def _sync_metadata_across_dp(
        ...
        #print("sync happend in torchair_model_runner.py")
        #sync here
        if self.enable_shared_expert_dp:
        ...
        if not with_prefill:
            ...
            maybe_padded_num_tokens = self.select_torchair_padded_batch_size(
                max_num_token)
            # print("real num_tokens_across_dp:", num_tokens_across_dp)
            # print("max num_tokens_across_dp:", num_tokens_across_dp.max().item())
            # print("maybe_padded_num_tokens:", maybe_padded_num_tokens)
            # print("self.torchair_graph_batch_sizes:", self.torchair_graph_batch_sizes)
    ...
    def _compile_torchair_graph(self, torchair_graph_batch_sizes) -> None:
        ...
        print("rank", self.dp_rank, "Start compiling torchair graph for batch sizes:", torchair_graph_batch_sizes)
