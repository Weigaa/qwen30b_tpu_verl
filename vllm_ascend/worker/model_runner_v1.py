    ...
    if attn_state != AscendAttentionState.DecodeOnly:
        ...
        for value in query_lens:
            ...
            if tokens >= total_tokens // 2:
                ...
                if abs(tokens - total_tokens // 2 - value) < total_tokens * imbalance_ratio:
...
class NPUModelRunner(GPUModelRunner):
    ...
    def __init__(self, vllm_config: VllmConfig, device: torch.device):
        ...
        # Attention graph task groups need a side stream to refresh dynamic
        # runtime params (seq_lens/block tables/etc.) before graph replay. This
        # is required by both FULL and PIECEWISE graph modes.
    ...
    def _skip_all_reduce_across_dp_group(self, is_draft_model=False) -> bool:
        ...
        """
        # Keep the legacy mode-0 behavior aligned with the old rollout stack:
        # every DP rank must participate in the same metadata all-reduce.
        if getattr(self.ascend_config, "elastic_execution_mode", 0) == 0:
            return False
    ...
    def _check_dbo_is_valid(self, query_lens: list[int],
                            attn_state: AscendAttentionState,
                            num_tokens: int) -> bool:
        if attn_state in [
        ...
        if (not self.vllm_config.model_config.use_mla
                or not envs_ascend.VLLM_ASCEND_ENABLE_DBO):
            return False
        token_index, seq_index = _compute_split_seq_index(
            query_lens, attn_state, num_tokens)
        if (token_index == 0 or seq_index == 0
                or seq_index == len(query_lens) or num_tokens < 256):
            return False
        return True
    ...
    def _prepare_inputs(
        ...
        enable_dbo = self._check_dbo_is_valid(self.query_lens.tolist(),
                                              self.attn_state,
                                              total_num_scheduled_tokens)
        self._elastic_local_enable_dbo = enable_dbo

        # Get info across DP ranks.
        (num_input_tokens, num_tokens_across_dp,
         with_prefill) = self._sync_metadata_across_dp(
             num_input_tokens, with_prefill, enable_dbo=enable_dbo)
        enable_dbo = bool(getattr(self, "_last_synced_enable_dbo", enable_dbo))
    ...
    def execute_model(
        ...
        if os.environ.get("VLLM_ASCEND_GRAPH_META_DEBUG", "0").lower() in (
                "1", "true", "yes", "on"):
            debug_count = getattr(self, "_graph_meta_debug_count", 0)
            debug_limit = int(
                os.environ.get("VLLM_ASCEND_GRAPH_META_DEBUG_LIMIT", "16"))
            if debug_count < debug_limit:
                try:
                    ...
                    tokens_across_dp = str(num_tokens_across_dp)
                try:
                    logits_head = logits_indices[:8].detach().cpu().tolist()
                except Exception:
                    logits_head = str(logits_indices[:8])
                logger.warning(
                    "graph_meta_debug rank=%s dp_rank=%s attn_state=%s "
                    "uniform_decode=%s acl_mode=%s num_input_tokens=%s "
                    "total_scheduled=%s num_actual_tokens=%s "
                    "num_tokens_across_dp=%s batch_descriptor_tokens=%s "
                    "batch_descriptor_reqs=%s max_query_len=%s "
                    "logits_indices_head=%s",
                    getattr(self, "rank", None),
                    getattr(self, "dp_rank", None),
                    getattr(getattr(self, "attn_state", None), "name",
                            str(getattr(self, "attn_state", None))),
                    uniform_decode,
                ...
                setattr(self, "_graph_meta_debug_count", debug_count + 1)
        ...
        with ProfileExecuteDuration().capture_async("forward"):
            with set_ascend_forward_context(
                ...
                if (aclgraph_runtime_mode == CUDAGraphMode.PIECEWISE
                        and not self.use_sparse
                        and os.getenv("VLLM_ASCEND_UPDATE_PIECEWISE_GRAPH_ATTN",
    ...
    def _dummy_run(
        ...
        baseline_mode0 = (
            getattr(self.ascend_config, "elastic_execution_mode", 0) == 0
            and self.dp_size > 1
            and os.environ.get("VLLM_ASCEND_ZIYI_GRAPH_DP_SYNC", "0").lower()
            not in ("1", "true", "yes", "on")
        )
        if baseline_mode0:
            # Align with the old baseline path: sync the raw dummy-run token
            # count first, then let every rank derive the same dispatched shape
            # from the synchronized token count.
            (num_tokens, num_tokens_across_dp,
             with_prefill) = self._sync_metadata_across_dp(
                 num_tokens, with_prefill, enable_dbo=False)
            ...
            (num_tokens, num_tokens_across_dp,
             with_prefill) = self._sync_metadata_across_dp(
                 batch_descriptor.num_tokens, with_prefill, enable_dbo=False)
