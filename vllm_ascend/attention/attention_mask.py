...
class AttentionMaskBuilder:
    ...
    def get_mask_scale_factor(dtype: torch.dtype = torch.float16):
        ...
        if dtype == torch.bfloat16:
            return -10000
        raise ValueError(
            "The current operation now only supports data types: "
            "torch.float16 and torch.bfloat16."
        )

    def get_legacy_splitfuse_attn_mask(
        self,
        seq_lens: torch.Tensor,
        position: torch.Tensor,
        dtype: torch.dtype,
    ) -> torch.Tensor:
        """Build the old eager splitfuse mask from per-token positions.

        The fixed 2048x2048 int8 mask used by the newer FIA path is not
        equivalent to the old splitfuse operator's expected mask layout.
        """
        if dtype not in (torch.float16, torch.bfloat16):
            raise ValueError("splitfuse_attn_mask only supports fp16 and bf16")
        ...
        if self.attn_mask_cache.dtype != dtype:
            self.attn_mask_cache = self.attn_mask_cache.to(dtype)
        # AttentionMaskBuilder is decorated by @singleton, so the module-level
        # name is a factory function at runtime. Use the instance method to keep
        # this path compatible with the singleton wrapper.
        mask_scale_factor = self.get_mask_scale_factor(dtype)
        if self.attn_mask_cache.device != self.device:
            self.attn_mask_cache = self.attn_mask_cache.to(
                self.device, non_blocking=True
            )
        index = position[: position.shape[0]]
        if index.device != self.attn_mask_cache.device:
            index = index.to(self.attn_mask_cache.device, non_blocking=True)
        attn_mask = torch.index_select(
            self.attn_mask_cache, dim=0, index=index
        )[:, :max_seq_len]
        attn_mask *= mask_scale_factor
        return attn_mask.contiguous().to(self.device, non_blocking=True)
