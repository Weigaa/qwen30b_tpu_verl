...
def build_redundant_replica_expert_map(global_expert_num,
    ...
    # Mode=1 floor-targeted redundancy needs exact loaded-slot capacity on
    # every rank. Integral replica rounds can be placed by cyclic shifts from
    # each expert's primary rank, preserving per-expert uniqueness while keeping
    # every rank's slot count aligned to the floor target.
    if (prefer_balanced_cyclic
        ...
        replica_rounds = max(int(global_redundant_expert_num),
                             0) // global_expert_num
        for replica_round in range(replica_rounds):
            shift = replica_round + 1
            for expert_id in range(global_expert_num):
                primary_rank = _determine_primary_rank(
                    global_expert_num, world_size, expert_id)
                ...
                if int(expert_map_all[target_rank, expert_id].item()) >= 0:
                    raise RuntimeError(
                        ...
                        f"primary_rank={primary_rank}, "
                        f"target_rank={target_rank}, "
                        f"replica_round={replica_round}.")
                expert_map_all[target_rank,
                               expert_id] = next_local_ids[target_rank]
