...
class Mode3DoubleBufferManager:
    ...
    def _ensure_slot_capacity(self, slot: _Mode3DoubleBufferSlot,
        ...
        # Mode 3: pre-allocate buffer based on VLLM_ASCEND_ELASTIC_MIN_COMPUTE_GROUP_SIZE.
        # The min compute group size determines the final shrink target, and we need
        # to reserve enough slots for the worst case (smallest group size).
        ...
        if is_mode3:
            ...
            # Keep the original behavior for other modes
...
class AscendFusedMoE(FusedMoE):
    ...
    def use_lossless_primary_reload_map_for_mode1(self) -> bool:
        ...
        if primary_log2phy is not None:
            ...
            self.primary_log2phy = self.log2phy.clone()
        elif self.log2phy is not None and self.log2phy.device != target_device:
            self.log2phy = self.log2phy.to(device=target_device)
            self.primary_log2phy = self.log2phy.clone()
        self._set_lossless_map_cpu_from_tensor(
            self.expert_map, "_lossless_expert_map_cpu")
    ...
    def export_lossless_expert_npu_weights(
        ...
        if len(local_slots) == 1:
            ...
            alias_w13 = self._lossless_p2p_alias_for_range(
                source_w13,
                int(local_slot),
                1,
            ...
            alias_w2 = self._lossless_p2p_alias_for_range(
                source_w2,
                int(local_slot),
                1,
