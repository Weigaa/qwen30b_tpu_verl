...
def check_npu_moe_gating_top_k(
        ...
        scoring_func: str = "softmax",
        custom_routing_function: Optional[Callable] = None):
    # Current Ascend fused moe_gating_top_k kernel only supports renorm=0.
    # Qwen3-MoE uses norm_topk_prob=True, so we must fall back to the
    # native routing path whenever renormalization is required.
    if renormalize:
        return False
