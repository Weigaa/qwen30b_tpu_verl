...
class MoECommMethod(ABC):
    ...
    def fused_experts(
        ...
        # print("log2phy in comm is", log2phy)
        # if num_tokens  == 32:
        #     self._t0 = torch.npu.Event(enable_timing=True)
        #     self._t1 = torch.npu.Event(enable_timing=True)
        #     self._t2 = torch.npu.Event(enable_timing=True)
        #     self._t3 = torch.npu.Event(enable_timing=True)
        ...
        if max_tokens < chunk_moe_size:
            ...
            #     print("this rank", self.ep_rank, "hidden size is", permuted_hidden_states.size(), "experts dis is", expert_tokens, "group_list size is", expert_tokens.size())
            if ~torch.any(expert_tokens):
                # print("do moe dummy train draft here")
            ...
            #     self._t3.record()
            #     torch.npu.synchronize()
            #     dispatch_ms = self._t0.elapsed_time(self._t1)
            #     gemm_ms     = self._t1.elapsed_time(self._t2)
            #     combine_ms  = self._t2.elapsed_time(self._t3)
            #     print("Fused Experts Time (ms): dispatch {:.3f} gemm {:.3f} combine {:.3f}".format(
            #         dispatch_ms, gemm_ms, combine_ms))
