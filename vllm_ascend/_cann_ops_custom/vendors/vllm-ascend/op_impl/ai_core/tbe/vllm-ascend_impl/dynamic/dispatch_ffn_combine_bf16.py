...
@tbe_register.register_operator("DispatchFFNCombineBF16", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.DYNAMIC_INPUT, para_check.DYNAMIC_INPUT, para_check.REQUIRED_INPUT, para_check.DYNAMIC_INPUT, para_check.DYNAMIC_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def dispatch_ffn_combine_bf16(a_in__, w1_in__, w2_in__, expertIdx_in__, scale1_in__, scale2_in__, probs_in__, out_out_, group, M=0, transB=False, weightNz=False, kernel_name="dispatch_ffn_combine_bf16", impl_mode = ""):
    ...
    custom_compile_options = {'__ALLSOC__': ['--cce-auto-sync=on', '-Wno-deprecated-declarations', '-Werror', '-DHCCL_COMM', '-I/usr/local/Ascend/cann-8.5.0/aarch64-linux/ascendc/include', '-I/workspace/vllm-ascend/csrc/third_party/catlass/include']},
    ...
    origin_func_name = "dispatch_ffn_combine_bf16"
    ascendc_src_dir_ex = "dispatch_ffn_combine_bf16"
    ascendc_src_dir = "dispatch_f_f_n_combine_b_f16"
    ascendc_src_file = "dispatch_ffn_combine_bf16.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Ascend C Operator DispatchFFNCombineBF16, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "DispatchFFNCombineBF16"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__ , impl_mode = impl_mode, origin_inputs=[a_in__, w1_in__, w2_in__, expertIdx_in__, scale1_in__, scale2_in__, probs_in__], origin_outputs = [out_out_],\
                param_type_dynamic = True, mc2_ctx = ["mc2_context_0"], param_type_list = ['required', 'dynamic', 'dynamic', 'required', 'dynamic', 'dynamic', 'required', 'required'], init_value_list = [None],\
...
def op_select_format(a_in__, w1_in__, w2_in__, expertIdx_in__, scale1_in__, scale2_in__, probs_in__, out_out_, group, M=0, transB=False, weightNz=False, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(a_in__, w1_in__, w2_in__, expertIdx_in__, scale1_in__, scale2_in__, probs_in__, out_out_, group, M, transB, weightNz)
    result = check_op_cap("op_select_format", "DispatchFFNCombineBF16", __inputs__, __outputs__, __attrs__)
...
def get_op_specific_info(a_in__, w1_in__, w2_in__, expertIdx_in__, scale1_in__, scale2_in__, probs_in__, out_out_, group, M=0, transB=False, weightNz=False, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(a_in__, w1_in__, w2_in__, expertIdx_in__, scale1_in__, scale2_in__, probs_in__, out_out_, group, M, transB, weightNz)
    result = check_op_cap("get_op_specific_info", "DispatchFFNCombineBF16", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
